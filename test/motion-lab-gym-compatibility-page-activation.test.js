...
const entryUrl = '/dev/motion-lab-assets/motion-lab-gym-compatibility-entry.js';
...
test('activates isolated gym compatibility entry after canonical bootstrap', () => {
  ...
  const entry = html.indexOf(entryUrl);
...
test('activates gym compatibility entry exactly once through protected asset route', () => {
  const matches = html.match(/\/dev\/motion-lab-assets\/motion-lab-gym-compatibility-entry\.js/g) || [];
  assert.equal(matches.length, 1);
  assert.doesNotMatch(html, /src="\/dev\/motion-lab-gym-compatibility-entry\.js"/);
});
