...
test('Phase 13 preserves its computed packet if the optional review integration throws',()=>{const phasePath=require.resolve('../public/mirror-motion-phase13');delete require.cache[phasePath];global.PocketPTMirrorMotionPhase2={diagnostics:()=>({trackerResets:0})};global.PocketPTMirrorMotionCameraReview={observe:()=>{throw new Error('review exploded');}};const p=require(phasePath);const e=p.createLateralIntentEngine({minCalibrationFrames:1});e.process(packet(0));const src=require('node:fs').readFileSync(phasePath,'utf8');assert.match(src,/CAMERA_MOTION_REVIEW_INTEGRATION_ERROR/);assert.match(src,/return out;/);delete global.PocketPTMirrorMotionPhase2;delete global.PocketPTMirrorMotionCameraReview;});
