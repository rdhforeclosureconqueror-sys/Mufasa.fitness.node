...
test('Phase 13 preserves computed lateral intent if optional camera review throws',()=>{const phasePath=require.resolve('../public/mirror-motion-phase13');delete require.cache[phasePath];global.PocketPTMirrorMotionPhase2={diagnostics:()=>({trackerResets:0})};global.PocketPTMirrorMotionCameraReview={observe:()=>{throw new Error('review exploded');}};const p=require(phasePath);for(let i=0;i<9;i++)p.process(packet(i));const out=p.process(packet(20));assert.ok(out.lateralIntent);assert.equal(out.lateralIntent.ready,true);const d=p.diagnostics();assert.equal(d.processErrors,0);assert.equal(d.cameraReviewErrors,10);assert.equal(d.firstFailingBoundary,'CAMERA_MOTION_REVIEW_INTEGRATION_ERROR');delete global.PocketPTMirrorMotionPhase2;delete global.PocketPTMirrorMotionCameraReview;});
