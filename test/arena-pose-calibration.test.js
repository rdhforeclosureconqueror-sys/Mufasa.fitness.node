...
function frame(kind, timestamp, jitter = 0) {
  ...
  return {timestamp, analysisUsable: true, trackingState: 'LOCKED', sequenceLandmarks: Object.fromEntries(
    Object.entries(points).map(([name, [x, y]]) => [name, {x: x + jitter, y, confidence: .95}]))};
}
...
test('captures stable personal TOP and BOTTOM geometry and confirms TOP return', () => {
  const stages = [], calibration = Calibration.create({onChange: state => stages.push(state.stage)});
  calibration.start(); hold(calibration, 'TOP');
  assert.deepEqual(calibration.snapshot(), {stage: 'CAPTURE_BOTTOM', topCaptured: true, bottomCaptured: false, calibrated: false});
  hold(calibration, 'BOTTOM', 800);
  assert.deepEqual(calibration.snapshot(), {stage: 'CONFIRM_TOP', topCaptured: true, bottomCaptured: true, calibrated: false});
...
test('does not capture an unchanged or unstable pose as BOTTOM', () => {
  const calibration = Calibration.create(); calibration.start(); hold(calibration, 'TOP');
...
test('rejects unusable frames and exposes no pose geometry in public state', () => {
  const calibration = Calibration.create(); calibration.start();
  ...
  calibration.reset(); assert.deepEqual(calibration.snapshot(), {stage: 'IDLE', topCaptured: false, bottomCaptured: false, calibrated: false});
});
