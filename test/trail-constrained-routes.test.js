...
test("empty and one-point geometry cannot pass corridor validation",()=>{for(const polyline of [[],[{latitude:32.94,longitude:-96.83}]])assert.equal(validateRouteCorridor({polyline,trailGraph:graph(),parkBoundary:boundary}).accepted,false);});
test("zero-length ways are excluded and cannot create graph loops",()=>{const value=buildPedestrianGraph([{type:"node",id:1,lat:32.94,lon:-96.83},{type:"way",id:20,nodes:[1,1],tags:{highway:"path"}}]);assert.equal(value.edges.length,0);assert.deepEqual(planTrailGraphRoute({targetDistanceMeters:800,graph:value,startPoint:value.nodes[1]}),[]);});
