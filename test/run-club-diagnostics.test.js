...
test("admin diagnostics UI provides an owner-scoped redacted Greatness verification trace",()=>{const html=fs.readFileSync(path.join(__dirname,"../public/admin-run-club-diagnostics.html"),"utf8"),js=fs.readFileSync(path.join(__dirname,"../public/admin-run-club-diagnostics.js"),"utf8");assert.match(html,/Inspect Most Recent Completed Activity/);assert.match(html,/Copy Redacted Verification Trace/);assert.match(js,/\/api\/me\/greatness\/journey/);assert.match(js,/verification-diagnostic/);assert.match(js,/traceAllowlist/);assert.doesNotMatch(js,/route\.points|latitude|longitude|userId|memberId/);assert.match(html,/greatness-verification-trace-2026\.08\.14\.1/);});
