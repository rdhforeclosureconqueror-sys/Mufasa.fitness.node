...
test("Run Club diagnostics shell and native HealthKit authorization require authoritative admin access",async t=>{const prior=process.env.AUTHZ_BOOTSTRAP_SUPER_ADMIN_USER_IDS;process.env.AUTHZ_BOOTSTRAP_SUPER_ADMIN_USER_IDS="runclub_admin";t.after(()=>prior==null?delete process.env.AUTHZ_BOOTSTRAP_SUPER_ADMIN_USER_IDS:process.env.AUTHZ_BOOTSTRAP_SUPER_ADMIN_USER_IDS=prior);const dataDir=fs.mkdtempSync(path.join(os.tmpdir(),"runclub-diag-api-")),app=createApp({dataDir}),server=app.listen(0);t.after(()=>server.close());await new Promise(r=>server.once("listening",r));const base=`http://127.0.0.1:${server.address().port}`,auth=createAuthTokenLib({secret:process.env.AUTH_TOKEN_SECRET||"dev-only-secret-change-me"}),member=auth.issueUserToken({userId:"ordinary_member"}).token,admin=auth.issueUserToken({userId:"runclub_admin"}).token,healthKitAuthorize=base+"/api/admin/diagnostics/healthkit/authorize";assert.equal((await fetch(base+"/admin-run-club-diagnostics.html")).status,200);assert.equal((await fetch(base+"/api/admin/diagnostics/run-club/run",{method:"POST"})).status,401);assert.equal((await fetch(base+"/api/admin/diagnostics/run-club/run",{method:"POST",headers:{authorization:`Bearer ${member}`}})).status,403);assert.equal((await fetch(healthKitAuthorize)).status,401);assert.equal((await fetch(healthKitAuthorize,{headers:{authorization:`Bearer ${member}`}})).status,403);const healthKitResponse=await fetch(healthKitAuthorize,{headers:{authorization:`Bearer ${admin}`}});assert.equal(healthKitResponse.status,200);assert.deepEqual((await healthKitResponse.json()).data,{authorized:true});const response=await fetch(base+"/api/admin/diagnostics/run-club/run",{method:"POST",headers:{authorization:`Bearer ${admin}`}});assert.equal(response.status,201);assert.equal((await response.json()).data.safeMode.cleanup,"automatic");});
