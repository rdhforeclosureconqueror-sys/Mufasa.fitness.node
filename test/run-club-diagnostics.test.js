...
test("authenticated admin navigation exposes Run Club Diagnostics without URL tokens",()=>{const html=fs.readFileSync(path.join(__dirname,"../public/dashboard.html"),"utf8"),js=fs.readFileSync(path.join(__dirname,"../public/dashboard.js"),"utf8"),page=fs.readFileSync(path.join(__dirname,"../public/admin-run-club-diagnostics.html"),"utf8");assert.match(html,/id="runClubDiagnosticsNav"[^>]+href="\/admin-run-club-diagnostics\.html"[^>]+hidden>Run Club Diagnostics/);assert.match(js,/refreshAuthStatus/);assert.match(js,/role === "admin" \|\| role === "super_admin"/);assert.doesNotMatch(html,/admin-run-club-diagnostics\.html\?[^"']*(token|auth)/i);assert.match(page,/Admin sign-in required/);assert.match(page,/Return to the Admin \/ Debug area/);});
