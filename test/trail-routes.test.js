...
test("trail UI separates internal details, directions, missing geometry, privacy, and mobile layout", () => {
  const js = fs.readFileSync(path.join(__dirname, "../public/greatness.js"), "utf8"), html = fs.readFileSync(path.join(__dirname, "../public/greatness.html"), "utf8"), css = fs.readFileSync(path.join(__dirname, "../public/greatness.css"), "utf8"), env = fs.readFileSync(path.join(__dirname, "../.env.example"), "utf8");
  assert.match(js, /View trail/); assert.match(js, /Directions to trailhead/); assert.match(js, /Trail route not yet verified/); assert.match(js, /showTrailDetails/); assert.doesNotMatch(js, /trailDiagnostic\([^\n]*(?:position\.coords|latitude|longitude)/); assert.match(html, /trailMap/); assert.match(css, /@media\(max-width:650px\)/); assert.match(env, /VITE_GOOGLE_MAPS_BROWSER_API_KEY/); assert.match(env, /GOOGLE_MAPS_API_KEY/);
});
