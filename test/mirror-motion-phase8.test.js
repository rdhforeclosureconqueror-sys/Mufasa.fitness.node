...
test('accepted facing intent is clamped and applied as rest-relative Y yaw without erasing existing root rotation',()=>{
  ...
  const expected=solver.rootRestQuaternion.clone()
    .multiply(new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0,1,0),65*Math.PI/180))
    .multiply(originalDelta).normalize();
...
test('weak intent holds prior yaw only for a bounded number of frames, then releases',()=>{
  ...
  for(let i=0;i<phase8.DEFAULTS.maxWeakHoldFrames;i++){
    assert.equal(phase8.chooseYaw({state:'SIDE',confidence:.2,yawIntentDeg:40,source:'hold',measuredDepth:false}),40);
  }
  assert.equal(phase8.chooseYaw({state:'SIDE',confidence:.2,yawIntentDeg:40,source:'hold',measuredDepth:false}),0);
});

test('claimed measured depth is rejected from yaw authority',()=>{
  phase8.reset();
  assert.equal(phase8.chooseYaw({state:'SIDE',confidence:.99,yawIntentDeg:50,source:'2d_projection',measuredDepth:true}),0);
  assert.equal(phase8.diagnostics().lastIssue,'DEPTH_CLAIM_REJECTED');
});
...
test('diagnostics expose first-failure and no-depth authority',()=>{
