...
test('validator persists only bounded needs_attention form findings and strips unknown raw fields',()=>{
  ...
  assert.throws(()=>validateFormFindings([{exerciseId:'squat',setIndex:0,ruleId:'knee_tracking',status:'good'}]),/needs_attention/);
});
...
test('workout tracking accepts deterministic form findings alongside ordinary completion facts',()=>{
  const tracking=validateWorkoutTracking({
    ...
    formFindings:[{exerciseId:'squat',setIndex:0,ruleId:'knee_tracking',status:'needs_attention',affectedFramePercentage:30,maximumConsecutiveDurationMs:600,confidence:.88}]
  });
...
test('dashboard completion bridge sends and clears current workout form findings only after successful persistence',()=>{
  ...
  const clear=source.indexOf('window.__POCKETPT_FORM_FINDINGS_CURRENT_WORKOUT__ = []');
  assert.ok(post>=0&&clear>post,'form-memory clear must happen after canonical workout persistence succeeds');
});
