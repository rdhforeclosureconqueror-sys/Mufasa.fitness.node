...
test('classification selects asymmetric wide-lunge, never symmetric straddle',()=>{const request={requestType:'motion-description-to-draft',exerciseId:'warrior-ii',displayName:'Warrior II',description:descriptions.descriptions.find(x=>x.exerciseId==='warrior-ii')};const out=Generator.generate(request,plans.plans.find(x=>x.exerciseId==='warrior-ii'));assert.equal(out.status,'ready');assert.equal(out.spec.generationMetadata.movementClassification.movementFamily,'standing-asymmetric-wide-lunge');assert.ok(out.spec.generationMetadata.selectedMovementOperators.some(x=>x.id==='lower-body:standing-asymmetric-wide-lunge-v1'));assert.ok(out.spec.generationMetadata.selectedMovementOperators.some(x=>x.id==='foot:lead-along-reach-rear-across-body'));assert.ok(out.spec.generationMetadata.selectedMovementOperators.some(x=>x.id==='head:gaze-over-lead-hand'));assert.ok(!out.spec.generationMetadata.selectedMovementOperators.some(x=>/straddle/.test(x.id)));assert.equal(out.spec.lowerBodySpatialPolicy.scaleReference,'effective-leg-length');assert.equal(out.spec.lowerBodySpatialPolicy.footOrientationPolicy.mode,'body-relative-foot-to-toe-yaw');assert.equal(out.spec.lowerBodySpatialPolicy.footOrientationPolicy.trailingTurnInDegrees,15);assert.deepEqual(out.spec.lowerBodySpatialPolicy.activePhaseIds,['target','hold']);assert.deepEqual(out.spec.phases.find(x=>x.id==='target').contacts,['left_foot','right_foot']);assert.deepEqual(out.spec.phases.find(x=>x.id==='hold').contacts,['left_foot','right_foot']);const gaze=out.spec.semanticPosePolicy.targets.find(x=>x.id==='head_gaze_lead_hand');assert.equal(gaze.type,'bone_yaw_toward_reference');assert.equal(gaze.targetReferenceBone,'LeftHand');});
...
test('mirrored wide-lunge semantics send the gaze and lead foot toward the right lead hand',()=>{const description=descriptions.descriptions.find(x=>x.exerciseId==='warrior-ii'),plan=plans.plans.find(x=>x.exerciseId==='warrior-ii'),rightDescription={...description,movementClassification:{...description.movementClassification,leadLeg:'right'}},rightPlan={...plan,movementClassification:{...plan.movementClassification,leadLeg:'right'}},out=Generator.generate({requestType:'motion-description-to-draft',exerciseId:'warrior-ii',displayName:'Warrior II',description:rightDescription},rightPlan);assert.equal(out.status,'ready');assert.equal(out.spec.lowerBodySpatialPolicy.leadLeg,'right');assert.equal(out.spec.semanticPosePolicy.targets.find(x=>x.id==='head_gaze_lead_hand').targetReferenceBone,'RightHand');});
...
test('generated Warrior II compiles and validates on the shipped personalized Avaturn avatar',async()=>{
  ...
  const gaze=compiled.diagnostics.semanticDirectionTargets.filter(x=>x.id==='head_gaze_lead_hand');
  assert.equal(gaze.length,2);
  assert.ok(gaze.every(x=>x.targetReference==='LeftHand'));
  assert.ok(gaze.every(x=>Math.abs(x.yawDegrees)>80&&Math.abs(x.yawDegrees)<100));
  assert.ok(gaze.every(x=>x.residualDegrees<.01));
  ...
  assert.equal(measured.leadFootOrientationStatus,'ready');
  assert.equal(measured.trailingFootOrientationStatus,'ready');
  assert.ok(measured.leadFootOrientationResidualDegrees<.01);
  assert.ok(measured.trailingFootOrientationResidualDegrees<.01);
});
