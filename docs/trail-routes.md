...
`VITE_GOOGLE_MAPS_BROWSER_API_KEY` is a server-runtime environment variable despite its historical `VITE_` name. This repository does not run a Vite static replacement step. The authenticated page requests `/api/browser-config`; the server returns only this public, browser-restricted key. Production operators must set it on the Node service, restart the service, enable billing and **Maps JavaScript API** (the loader also requests the geometry library), and allow the exact production HTTPS origins/referrers, including both apex and `www` forms when both are served. Never copy `GOOGLE_MAPS_API_KEY` into this variable.
