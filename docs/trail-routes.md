...
`VITE_GOOGLE_MAPS_BROWSER_API_KEY` keeps its established frontend deployment contract. The loader first accepts the frontend-injected runtime value, then requests the frontend's same-origin `/api/browser-config`, matching the original implementation in `7f390f1`. The configured backend-origin endpoint is retained only as a compatibility fallback. This ordering repairs the regression introduced in `988c795`, which bypassed the same-origin frontend path and always selected the backend origin. The endpoint returns only this public, browser-restricted key. Operators do not need to move it to the Places service. Enable billing and **Maps JavaScript API** (the loader also requests the geometry library), and allow the exact production HTTPS origins/referrers, including both apex and `www` forms when both are served. Never copy `GOOGLE_MAPS_API_KEY` into this variable.
