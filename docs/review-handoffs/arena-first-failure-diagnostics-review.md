...
Camera access is explicit. The canonical CameraController, MoveNet runtime and PoseCaptureEngine provide the camera check. During a real mat setup, fresh pose frames feed an attempt-scoped calibration helper: it automatically captures a stable personal TOP geometry template, a stable distinct BOTTOM template, then requires a return to TOP. It retains derived joint-angle geometry only in memory, never saves photographs, and never sends landmarks to Godot or the backend. Calibration proves two personal signal gates and their order; it does not certify safe form or approved depth. Full start-position/rep rules remain draft/unconnected, so this implementation cannot start a countdown, count official challenge reps, save a score or update a leaderboard. Coach audio and the actual coach avatar are not implemented here; the briefing is captioned.
...
| `public/arena-pose-calibration.js` | Attempt-only stable TOP/BOTTOM geometry capture, TOP return confirmation and private gate classification; no images, persistence or official form claim. |
...
| `test/arena-pose-calibration.test.js` | Stable/distinct capture, TOP return, unusable-frame rejection, gate classification and public-state privacy. |
...
Expected full PR scope: nineteen files. No `public/game/push-up-arena/*`, generated `data/ops/`, Godot scenes/animations, server/auth services, exercise rules, scores or deployment configuration may change.
...
4. **Evidence:** no sender means unconnected, no capability means disabled touch, and BODY_VISIBLE cannot imply valid posture/countdown/reps. TOP/BOTTOM calibration may pass only after stable attempt-scoped geometry and a confirmed TOP return; it must not turn START_POSITION, READY_GESTURE, REP_DETECTOR or TIMER green. Fallback never becomes personal-avatar success. Caption briefing never becomes voice PASS.
...
7. **Camera/calibration:** no getUserMedia/model download on launch. Use the existing controller/capture/runtime. Check denied and unanswered permission, late grants after cancel, model completion after exit, camera switching, track end, stale frames, orientation and hide. Calibration must reject low-confidence, stale, unstable and unchanged candidate poses; reset on camera replacement/exit; keep templates in memory only; and require TOP → distinct BOTTOM → TOP. Only one detector per session; no second inference owner, recorder, profile store or body-data bridge.
...
The implementer's current result is **124 passing tests**, zero failures/skips, using:

```powershell
npm ci --ignore-scripts --no-audit --no-fund
node --test --test-reporter=spec test/arena-pose-calibration.test.js test/arena-phone-flow.test.js test/arena-camera.test.js test/arena-phone-ui.test.js test/arena-diagnostics.test.js test/world-bridge-pocketpt-finish.test.js test/world-bridge-production-entry.test.js test/world-bridge-mobile-auth.test.js test/world-bridge.test.js test/world-avatar-bridge.test.js test/push-up-challenge-mvp.test.js test/push-up-tracking-continuity.test.js
...
| `phone-flow` | Go to mat → simulated arrival → caption briefing → Set up my camera → Enable camera → simulated body visibility and TOP-capture instruction. The fixture does not synthesize exercise poses, so calibration cannot falsely complete. No countdown. Return waits for simulated standing. Thumb controls send commands. |
...
4. Check framing guidance and BODY_VISIBLE with current joints. In a real mat setup, hold TOP, then BOTTOM, then return to TOP. The UI and diagnostic rows should advance separately. Losing visibility or holding an unchanged pose must not complete calibration. Calibration must not start a countdown, score movement or label the person ready for a technically valid push-up.
