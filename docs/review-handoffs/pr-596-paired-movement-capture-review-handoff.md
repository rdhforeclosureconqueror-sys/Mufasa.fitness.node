...
This follow-up begins after merged PR #595 and contains only the additional paired-view/custom-movement/pose-checkpoint work added after that merge.

## GOAL
Upgrade the trainer Movement Lego Recorder so foundational and custom movements can be collected as paired FRONT + SIDE MoveNet evidence, then annotated with timestamped 2D skeleton pose checkpoints.
...
2. Verify the boot chain is deterministic: recorder -> roadmap -> capture studio.
...
Then run relevant existing pose/workout tests if practical. Do not claim a test passed unless actually executed.

## LIVE ACCEPTANCE
1. Open trainer/admin Coach Demo Exercise Template Builder.
2. Confirm Recorder + Roadmap + Paired View Studio appear.
3. Load Slow Bodyweight Squat.
4. Confirm FRONT is requested first when no evidence exists.
5. Record/save front and verify Front ✓ / Side □ plus explicit SIDE prompt.
6. Record/save side and verify Front ✓ / Side ✓ / paired complete.
7. Verify skeleton checkpoint cards render with timestamp and frame index.
8. Create `One-Arm Push-Up Left` custom movement.
9. Verify it enters the recorder and requires front then side.
10. Export JSON and verify no raw video/image payload is included. Generated skeleton SVG strings are acceptable.
11. Verify existing template demo capture and normal camera/avatar/workout behavior still work.
