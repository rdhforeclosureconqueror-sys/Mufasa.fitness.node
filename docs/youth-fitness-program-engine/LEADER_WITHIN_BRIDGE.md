...
* Pocket PT authentication resolves the subject on the server as `req.auth.userId`. Youth runtime routes are `/api/me/...`; their write operations use a subject-bound, double-submit CSRF token. The existing credential-free launch page is `/pocketpt/my-program`.
...
The youth launch value is the fixed same-application path `/pocketpt/my-program`; it carries no participant identifier, credential, session token, or CSRF token. Opening it does not mutate completion. Pocket PT readiness, Phase 6 validation, execution, and canonical completion remain authoritative.
...
Focused tests use synthetic identities and prove mapping uniqueness, program-first linkage, happy-path completion/credit/progress, safety hold, minimum-data projection, ownership and organization denial, facilitator permission, outage behavior, unrelated-session rejection, idempotency, and preservation after repository reopen. No live Leader Within deployment, production SSO, staging, browser end-to-end session, real facilitator UI, or live user was available or verified.
