(function (root, factory) {
  ...
  const rows = [
    ...
    ['MIRROR_MOTION_INPUT', 'Mirror Motion pose input', 'Body tracking', ['BODY_VISIBILITY'], 'PocketPT', 'Verify the existing Arena detector frame reaches the canonical Mirror Motion adapter.'],
    ['REST_BASE_CAPTURE', 'Mirror Motion rest/base', 'Body tracking', ['MIRROR_MOTION_INPUT'], 'PocketPT', 'Complete the protected neutral baseline; exercise TOP/BOTTOM calibration remains separate.'],
    ['MIRROR_MOTION_READY', 'Processed Mirror Motion pose', 'Body tracking', ['REST_BASE_CAPTURE'], 'PocketPT', 'Verify a valid rest-relative processed pose, not merely loaded code.'],
    ['MOCAP_BRIDGE', 'LIVE_MOCAP bridge send', 'Body tracking', ['MIRROR_MOTION_READY', 'POSE_CYCLE_CALIBRATION', 'CONTROL_CHANNEL'], 'Bridge', 'Verify a current versioned packet was sent only after TOP/BOTTOM/TOP calibration.'],
    ['GODOT_LIVE_MOCAP', 'Godot LIVE_MOCAP ownership', 'Body tracking', ['MOCAP_BRIDGE', 'GODOT_REPORTER'], 'Godot', 'Implement and report exclusive LIVE_MOCAP ownership in the Godot source.'],
    ['MOCAP_BONE_BIND', 'Personalized mocap bone bind', 'Body tracking', ['GODOT_LIVE_MOCAP', 'AVATAR_MOUNT'], 'Godot', 'Resolve canonical joints through the saved gymMappingProfile.'],
    ['MOCAP_FRAME_APPLIED', 'LIVE_MOCAP frame applied', 'Body tracking', ['MOCAP_BONE_BIND'], 'Godot', 'Report an accepted fresh frame applied to mapped bones.'],
    ['LIVE_AVATAR_VISUAL_READY', 'Personal avatar visibly live', 'Body tracking', ['MOCAP_FRAME_APPLIED', 'AVATAR_MOUNT'], 'Godot', 'Requires runtime render evidence; visible motion quality remains a physical human check.'],
  ...
  const DETAILS = Object.freeze({
    ...
    MIRROR_INPUT_WAITING: 'Waiting for the first current Arena pose packet.',
    MIRROR_INPUT_OBSERVED: 'The existing Arena detector packet reached the canonical Mirror Motion input.',
    MIRROR_INPUT_STALE: 'The Arena pose input is missing or stale; readiness is not retained.',
    MIRROR_RUNTIME_MISSING: 'The canonical Mirror Motion runtime did not load.',
    REST_BASE_CAPTURING: 'The protected Mirror Motion neutral baseline is being captured.',
    REST_BASE_READY: 'The protected Mirror Motion rest/base calibration completed.',
    MIRROR_REST_NOT_READY: 'Mirror Motion is withholding LIVE_MOCAP until rest/base calibration completes.',
    MIRROR_OUTPUT_READY: 'Mirror Motion produced a current rest-relative canonical pose.',
    MIRROR_OUTPUT_INVALID: 'Mirror Motion produced no trustworthy mapped joint rotations.',
    MIRROR_PROCESSING_FAILED: 'Mirror Motion rejected or failed to process the current detector packet.',
    MOCAP_FRAME_SENT: 'A versioned processed pose packet was sent through the existing bridge.',
    PUSHUP_CALIBRATION_PENDING: 'Rest/base is ready; LIVE_MOCAP waits for a valid TOP to BOTTOM to TOP exercise calibration.',
    MOCAP_SEND_REJECTED: 'The existing bridge rejected the processed pose packet.',
    MOCAP_RELEASED: 'LIVE_MOCAP is released; Godot must retain locomotion/world authority.',
    COACH_CUE_QUEUED: 'A calibration cue was queued through the canonical CoachRuntime speech owner.',
