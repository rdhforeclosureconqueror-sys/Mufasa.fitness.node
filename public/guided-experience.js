(function installPocketPtGuide(global) {
  ...
  const tours = {
    ...
    intake: [
      ...
      { target: '[data-tour="intake-goals"]', optional: true, title: "Your goals", body: "Share what you want to work toward so your recommendations can focus on what matters to you." },
      { target: '[data-tour="intake-readiness"]', optional: true, title: "Training history and readiness", body: "Tell Pocket PT about your experience and readiness. If a review is needed, it happens before you progress into training." },
      { target: '[data-tour="intake-movement"]', optional: true, title: "Movement check", body: "Complete the available movement questions or assessment so your starting point can be better matched to you." },
      { target: '[data-tour="intake-preferences"]', optional: true, title: "Schedule, equipment, and preferences", body: "Your available time, equipment, and preferences help shape a plan you can realistically follow." },
      { target: '[data-tour="intake-complete"]', optional: true, title: "Finish and review", body: "Complete the Intake to move toward your personalized program. Pocket PT does not diagnose or provide medical treatment." }
    ],
    training: [
    ...
    challenge: [
      { target: '[data-tour="challenge-overview"]', title: "Your challenge", body: "See the current week and the focus of your active challenge." },
      { target: '[data-tour="challenge-progress"]', title: "Commitment progress", body: "Review your commitment percentage, completed workouts, and week streak." },
      { target: '[data-tour="challenge-workout"]', optional: true, title: "Today’s challenge workout", body: "Review the session and start or resume it when you are ready." },
      { target: '[data-tour="challenge-schedule"]', optional: true, title: "Your weekly schedule", body: "See planned workout and recovery days, and adjust an available workout day when needed." }
    ],
    "exercise-library": [
  ...
  const contexts = [
    ...
    { match: /yoga\.html$/, tourId: "yoga" }, { match: /challenge\.html$/, tourId: "challenge" },
  ...
  async function show() {
    ...
    if (step?.route && !routeMatches(step.route)) { navigateForTour(active.id === "introduction" ? "intake" : active.id, step.route); remove(); return true; }
    let target = await waitForTarget(step.target);
    while (!target && step?.optional && ++active.index < active.steps.length) { step = active.steps[active.index]; target = await waitForTarget(step.target); }
    if (!target) { active.failed = true; remove(); return false; }
  ...
  async function start(id, { manual = false } = {}) { if (!tours[id] || active) return false; if (!manual && document.querySelector('dialog[open],[role="dialog"][aria-modal="true"]')) return false; previousFocus = document.activeElement; const state = await api(); if (!manual && (state?.proactiveDisabled || state?.tours?.[id]?.dismissed || state?.tours?.[id]?.completedAt)) return false; active = { id, index: 0, steps: tours[id], manual }; abort = new AbortController(); global.addEventListener("resize", () => active && placeCard(document.querySelector(".ppt-guide-card"), document.querySelector(".ppt-guide-target")), { signal: abort.signal }); return show(); }
  ...
  function initialize() { if (listeners) return; listeners = true;
    ...
    document.addEventListener("click", async event => { const layer = event.target.closest(".ppt-guide-layer"); if (event.target.closest(".ppt-guide-close")) { active ? await finish("dismiss") : remove(); return; } const action = event.target.closest("[data-guide]")?.dataset.guide; if (action === "back" && active) { active.index--; show(); } if (action === "next" && active) { if (active.index === active.steps.length - 1) await finish("complete"); else { active.index++; show(); } } if (action === "skip") await finish("dismiss"); const id = event.target.closest("[data-tour-id]")?.dataset.tourId; if (id) { remove(); start(id, { manual: true }); } if (event.target.closest("[data-pocketpt-guides]")) openHelp(); if (event.target.closest("[data-prompt-dismiss]")) { await api("PATCH", { promptKey: layer.dataset.promptKey, action: "dismiss-prompt" }); remove(); } if (event.target.closest("[data-prompt-disable]")) { await api("PATCH", { proactiveDisabled: true }); remove(); } if (event.target.closest("[data-prompt-guide]")) { const id = layer.dataset.tourId, route = layer.dataset.route; remove(); if (route && !routeMatches(route)) navigateForTour(id, route); else start(id, { manual: true }); } });
  ...
  global.PocketPTGuide = { initialize, start, showJourney, openHelp, dispose: remove, waitForTarget, tours, contexts, get active() { return active; } }; initialize();
})(window);
