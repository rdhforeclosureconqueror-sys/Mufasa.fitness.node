(function initMirrorMotionPhase4(root, factory) {
  ...
  const angle = (a,b,c) => {
    ...
    const bax = Number(a.x)-Number(b.x), bay = Number(a.y)-Number(b.y);
    const bcx = Number(c.x)-Number(b.x), bcy = Number(c.y)-Number(b.y);
  ...
  function bodyScale(map) {
    const shoulders = dist(map.get('left_shoulder'), map.get('right_shoulder'));
    const hips = dist(map.get('left_hip'), map.get('right_hip'));
    const values = [shoulders,hips].filter(Number.isFinite).filter(v => v > 1);
  ...
  function createExerciseContextEngine(options = {}) {
    ...
    function classify(map, requestedPattern) {
      ...
      const lk=map.get('left_knee'), rk=map.get('right_knee'), la=map.get('left_ankle'), ra=map.get('right_ankle');
      const lw=map.get('left_wrist'), rw=map.get('right_wrist');
      ...
      const horizontal = torso ? Math.abs(torso.shoulder.x-torso.hip.x) > Math.abs(torso.shoulder.y-torso.hip.y) * config.horizontalTorsoRatio : false;
      const leftKnee = angle(lh,lk,la), rightKnee = angle(rh,rk,ra);
      const knee = [leftKnee,rightKnee].filter(Number.isFinite);
      const kneeAvg = knee.length ? knee.reduce((a,b)=>a+b,0)/knee.length : NaN;
      const ankleSep = dist(la,ra);
      const shoulderSep = dist(ls,rs);
      ...
      if (requestedPattern === 'pushup') {
        if (horizontal) return { phase:'PUSHUP_HORIZONTAL', scale, kneeAvg };
        return { phase:'PUSHUP_TRANSITION', scale, kneeAvg };
      }
      ...
      if (horizontal) return { phase:'HORIZONTAL_BODY', scale, kneeAvg };
      return { phase:'UPRIGHT_BODY', scale, kneeAvg };
    }

    function setAnchor(name, point) {
      if (!usable(point,config.minConfidence)) return;
      anchors[name] = { x:Number(point.x), y:Number(point.y) };
    }

    function releaseAnchors(reason) {
      if (Object.keys(anchors).length) anchorReleases += 1;
      anchors = {};
      lastIssue = `ANCHORS_RELEASED:${reason}`;
    }
    ...
    function applyAnchor(map, name, scale, stats) {
      ...
      const drift = Math.hypot(Number(point.x)-anchor.x, Number(point.y)-anchor.y);
      const maxDrift = scale * config.anchorMaxDriftRatio;
      if (drift > maxDrift) {
        delete anchors[name];
        anchorReleases += 1;
        lastIssue = `ANCHOR_RELEASED_EXCESS_DRIFT:${name}`;
        return;
      }
      ...
      point.exerciseConstraintState = 'contact_anchor';
      point.exerciseAnchor = name;
      stats.anchorCorrections += 1; anchorCorrections += 1;
      lastIssue = `CONTACT_ANCHORED:${name}`;
    }
    ...
    function process(packet, requestedPattern = null) {
      ...
      if (pattern && nextPattern && pattern !== nextPattern) releaseAnchors('EXERCISE_CHANGED');
      ...
      if (pattern === 'squat') {
        ...
        applyAnchor(map,'left_ankle',classification.scale,stats);
        applyAnchor(map,'right_ankle',classification.scale,stats);
        ...
        releaseAnchors('NON_ANCHORED_EXERCISE');
      }
      ...
      return {
        ...
        exerciseContext: {
          ...
          phase,
          bodyScalePx:Number.isFinite(classification.scale) ? classification.scale : null,
          kneeAngleDeg:Number.isFinite(classification.kneeAvg) ? classification.kneeAvg : null,
          frameStats:stats,
          anchors:{ ...anchors },
          lastIssue
        }
  ...
  function updateRuntimeDiagnostics(extra={}) {
    ...
    Object.assign(runtime, {
      mirrorMotionPhase:4,
      mirrorMotionPhase4Installed:state.installed,
      mirrorMotionExercisePatched:state.avatarRuntimePatched,
      mirrorMotionExercisePattern:diag.pattern,
      mirrorMotionExercisePhase:diag.phase,
      mirrorMotionExerciseAnchoredContacts:diag.anchoredContacts,
      mirrorMotionExerciseAnchorCorrections:diag.anchorCorrections,
      mirrorMotionExerciseAnchorReleases:diag.anchorReleases,
      mirrorMotionExerciseLastIssue:diag.lastIssue,
      mirrorMotionExerciseFirstFailingBoundary:state.firstFailingBoundary,
      mirrorMotionExerciseProcessErrors:state.processErrors,
      ...extra
    });
    globalScope.__mirrorMotionPhase4Diagnostics = { ...state, ...diag };
  }
  ...
  function wrapRenderer(renderer) {
    ...
    return function exerciseAwareRenderer(structuralPacket) {
      ...
      try {
        const constrained = engine.process(structuralPacket);
        state.firstFailingBoundary='NONE';
        state.lastPipelineStage='EXERCISE_CONTEXT_READY';
        updateRuntimeDiagnostics();
        return renderer(constrained);
      } catch (error) {
        state.processErrors += 1;
        state.firstFailingBoundary='EXERCISE_CONTEXT_PROCESS_ERROR';
        state.lastPipelineStage='EXERCISE_CONTEXT_PROCESS_ERROR';
        updateRuntimeDiagnostics({ mirrorMotionExerciseLastError:String(error?.message || error) });
        return renderer(structuralPacket);
      }
    };
  }
  ...
  function patchAvatarRuntime(runtime) {
    ...
    if (typeof originalBind !== 'function') {
      state.firstFailingBoundary='PHASE4_AVATAR_BIND_API_MISSING'; updateRuntimeDiagnostics(); return runtime;
    }
    ...
    Object.defineProperty(runtime,'__mirrorMotionPhase4Patched',{ value:true, enumerable:false });
    state.avatarRuntimePatched=true; state.firstFailingBoundary='NONE'; state.lastPipelineStage='AVATAR_RUNTIME_PATCHED'; updateRuntimeDiagnostics();
    return runtime;
  }
  ...
  function interceptAvatarRuntimeAssignment() {
    ...
    Object.defineProperty(globalScope,'AvatarRuntime',{
      configurable:true, enumerable:prior?.enumerable !== false,
      get(){ return prior?.get ? prior.get.call(globalScope) : fallbackValue; },
      set(next){
        if (prior?.set) prior.set.call(globalScope,next); else fallbackValue=next;
        const current = prior?.get ? prior.get.call(globalScope) : fallbackValue;
        const patched = patchAvatarRuntime(current || next);
        if (!prior?.set) fallbackValue=patched;
      }
    });
  }
  ...
  function diagnosticsText() {
    ...
    return [
      'MIRROR MOTION INTELLIGENCE — PHASE 4',
      `First failing boundary: ${state.firstFailingBoundary}`,
      `Pipeline stage: ${state.lastPipelineStage}`,
      `Runtime patched: ${state.avatarRuntimePatched ? 'YES':'NO'}`,
      `Renderer bound: ${state.rendererBound ? 'YES':'NO'}`,
      `Exercise pattern: ${d.pattern}`,
      `Exercise phase: ${d.phase}`,
      `Anchored contacts: ${d.anchoredContacts}`,
      `Anchor corrections: ${d.anchorCorrections}`,
      `Anchor releases: ${d.anchorReleases}`,
      `Last exercise issue: ${d.lastIssue}`,
      `Process errors: ${state.processErrors}`
    ].join('\n');
  }

  function ensureDebugPanel() {
    const doc=globalScope.document; if (!doc?.body) return null;
    let panel=doc.getElementById('mirrorMotionPhase4Debug');
    if (!panel) {
      panel=doc.createElement('details'); panel.id='mirrorMotionPhase4Debug';
      panel.style.cssText='position:fixed;right:8px;bottom:8px;z-index:5002;max-width:min(92vw,520px);max-height:42vh;overflow:auto;background:rgba(2,6,23,.94);color:#e5e7eb;border:1px solid #38bdf8;border-radius:10px;padding:8px;font:12px/1.35 ui-monospace,SFMono-Regular,Menlo,monospace;';
      const summary=doc.createElement('summary'); summary.textContent='Mirror Motion Phase 4 Debug'; summary.style.cssText='cursor:pointer;color:#7dd3fc;font-weight:700;';
      const pre=doc.createElement('pre'); pre.dataset.mirrorMotionPhase4Diagnostics='true'; pre.style.cssText='white-space:pre-wrap;margin:8px 0 0;';
      panel.append(summary,pre); doc.body.appendChild(panel);
    }
    return panel;
  }
  ...
  function install() {
    ...
    state.installed=true; state.lastPipelineStage='INSTALLING'; interceptAvatarRuntimeAssignment();
    const mount=()=>{ ensureDebugPanel(); refreshDebugPanel(); if(!panelTimer && globalScope.setInterval) panelTimer=globalScope.setInterval(refreshDebugPanel,500); };
    if(globalScope.document?.readyState==='loading') globalScope.document.addEventListener('DOMContentLoaded',mount,{once:true}); else mount();
    state.lastPipelineStage='INSTALLED'; updateRuntimeDiagnostics(); return api;
  }
