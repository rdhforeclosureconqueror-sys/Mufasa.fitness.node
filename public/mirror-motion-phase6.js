(function initMirrorMotionPhase6(root,factory){
  ...
  const DEFAULTS=Object.freeze({
    ...
    velocityGain:0.24,accelerationGain:0.004,
  ...
  function createLiveCurveEngine(options={}){
    ...
    let frames=0,tunedPoints=0,anchorBypasses=0,ikBypasses=0,uncertainBypasses=0,scaleBypasses=0,resets=0,lastPattern=null,lastTimestamp=null;
    ...
    function process(packet){
      ...
      if(lastPattern&&pattern!==lastPattern){reset('EXERCISE_CHANGED');lastTimestamp=now;}
      lastPattern=pattern;
      ...
      const frameStats={tunedPoints:0,anchorBypasses:0,ikBypasses:0,uncertainBypasses:0,scaleBypasses:0};
      let frameAlphaSum=0,frameAlphaSamples=0,frameSuppressionSum=0,frameLatencySum=0;
      const points=(packet?.keypoints||[]).map(source=>{
        const p={...source},name=pointName(p);
        if(!name||!finite(p.x)||!finite(p.y))return p;
        if(['dropped','coasted'].includes(p.stabilityState)){
          history.delete(name);uncertainBypasses+=1;frameStats.uncertainBypasses+=1;p.curveState='uncertain_passthrough';return p;
        }
        if(p.ikState==='solved'){
          history.set(name,{x:Number(p.x),y:Number(p.y),vx:0,vy:0});ikBypasses+=1;frameStats.ikBypasses+=1;p.curveState='ik_passthrough';return p;
        }
        if(anchored[name]){
          history.set(name,{x:Number(p.x),y:Number(p.y),vx:0,vy:0});anchorBypasses+=1;frameStats.anchorBypasses+=1;p.curveState='anchor_passthrough';return p;
        }
        ...
        const acceleration=Math.hypot(rawVx-prev.vx,rawVy-prev.vy)/(scale*dt);
        ...
        tunedPoints+=1;frameStats.tunedPoints+=1;
        alphaSum+=alpha;alphaSamples+=1;suppressionSum+=suppression;latencySum+=estimatedLatencyMs;
        frameAlphaSum+=alpha;frameAlphaSamples+=1;frameSuppressionSum+=suppression;frameLatencySum+=estimatedLatencyMs;
        return p;
      });
      const frameAverageAlpha=frameAlphaSamples?frameAlphaSum/frameAlphaSamples:null;
      const frameAverageSuppressionPx=frameAlphaSamples?frameSuppressionSum/frameAlphaSamples:0;
      const frameEstimatedLatencyMs=frameAlphaSamples?frameLatencySum/frameAlphaSamples:0;
      lastIssue=scale==null?'BODY_SCALE_UNAVAILABLE':'CURVE_APPLIED';
      return {...packet,keypoints:points,liveCurve:{version:3,frame:frames,pattern,dtMs,frameStats,averageAlpha:frameAverageAlpha,averageSuppressionPx:frameAverageSuppressionPx,estimatedLatencyMs:frameEstimatedLatencyMs,lastIssue}};
    }
    function diagnostics(){return{frames,tunedPoints,anchorBypasses,ikBypasses,uncertainBypasses,scaleBypasses,resets,historyPoints:history.size,averageAlpha:alphaSamples?alphaSum/alphaSamples:null,averageSuppressionPx:alphaSamples?suppressionSum/alphaSamples:0,estimatedLatencyMs:alphaSamples?latencySum/alphaSamples:0,lastIssue};}
