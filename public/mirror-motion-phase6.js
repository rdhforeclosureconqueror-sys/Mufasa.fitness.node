(function initMirrorMotionPhase6(root,factory){
  ...
  const packetTime=(packet,fallback)=>{
    for(const value of [packet?.timestampMs,packet?.timestamp,packet?.timeMs,packet?.ts]){const n=Number(value);if(Number.isFinite(n))return n;}
    return fallback;
  };
  ...
  function createLiveCurveEngine(options={}){
    ...
    let frames=0,tunedPoints=0,anchorBypasses=0,uncertainBypasses=0,resets=0,lastPattern=null,lastTimestamp=null;
    let alphaSum=0,alphaSamples=0,suppressionSum=0,latencySum=0,lastIssue='NONE';

    function reset(reason='MANUAL'){
      history.clear();lastTimestamp=null;lastPattern=null;resets+=1;lastIssue=`CURVE_RESET:${reason}`;
    }
    function phaseBias(packet){
      const phase=String(packet?.exerciseContext?.phase||'');
      if(/TRANSITION/.test(phase))return 0.10;
      if(/JACK_OPEN|JACK_CLOSED/.test(phase))return 0.08;
      if(/BOTTOM/.test(phase))return -0.06;
      return 0;
    }
    function jointBias(name){
      if(config.responsiveJoints.includes(name))return 0.08;
      if(config.stableJoints.includes(name))return -0.05;
      return 0;
    }
    function process(packet){
      ...
      lastPattern=pattern;
      const scale=Number(packet?.exerciseContext?.bodyScalePx)||Number(packet?.structural?.bodyScalePx)||100;
      ...
      const points=(packet?.keypoints||[]).map(source=>{
        ...
        if(anchored[name]){history.set(name,{x:Number(p.x),y:Number(p.y),vx:0,vy:0});anchorBypasses+=1;p.curveState='anchor_passthrough';return p;}
        if(['dropped','coasted'].includes(p.stabilityState)){uncertainBypasses+=1;p.curveState='uncertain_passthrough';return p;}
        ...
        const speed=Math.hypot(rawVx,rawVy)/Math.max(scale,1);
        const acceleration=Math.hypot(rawVx-prev.vx,rawVy-prev.vy)/Math.max(scale,1);
        ...
        const lagDistance=Math.hypot(Number(p.x)-outX,Number(p.y)-outY);
        const rawSpeedPx=Math.hypot(rawVx,rawVy);
        ...
        tunedPoints+=1;alphaSum+=alpha;alphaSamples+=1;suppressionSum+=suppression;latencySum+=estimatedLatencyMs;
        return p;
      });
      lastIssue='CURVE_APPLIED';
      return {...packet,keypoints:points,liveCurve:{version:1,frame:frames,pattern,dtMs,frameStats:{tunedPoints,anchorBypasses,uncertainBypasses},averageAlpha:alphaSamples?alphaSum/alphaSamples:null,averageSuppressionPx:alphaSamples?suppressionSum/alphaSamples:0,estimatedLatencyMs:alphaSamples?latencySum/alphaSamples:0,lastIssue}};
    }
    function diagnostics(){return{frames,tunedPoints,anchorBypasses,uncertainBypasses,resets,historyPoints:history.size,averageAlpha:alphaSamples?alphaSum/alphaSamples:null,averageSuppressionPx:alphaSamples?suppressionSum/alphaSamples:0,estimatedLatencyMs:alphaSamples?latencySum/alphaSamples:0,lastIssue};}
