...
export async function renderTrailMap(container, payload, { onSelect = () => {}, onDiagnostic = () => {}, selectedId = null, geometry = null, turnaroundPoint = null, routeSource = "verified_geometry" } = {}) {
  ...
  if (geometry?.length > 1) { const path = geometry.map(point => ({ lat: point.latitude, lng: point.longitude })),generated=routeSource==="google_walking_route"; path.forEach(point => bounds.extend(point)); new google.maps.Polyline({ map, path, strokeColor: generated ? "#58a6ff" : "#f1c963", strokeOpacity:generated?0.9:1, strokeWeight: generated ? 6 : 5,icons:generated?[{icon:{path:"M 0,-1 0,1",strokeOpacity:1,scale:3},offset:"0",repeat:"18px"}]:undefined }); new AdvancedMarkerElement({ map, position: path[0], title: "Route start", content: new PinElement({ glyph: "S", background: "#188038" }).element }); if(turnaroundPoint)new AdvancedMarkerElement({map,position:{lat:turnaroundPoint.latitude,lng:turnaroundPoint.longitude},title:"Turn around",content:new PinElement({glyph:"↩",background:"#fbbc04",glyphColor:"#172019"}).element});new AdvancedMarkerElement({ map, position: path[path.length - 1], title: "Route finish", content: new PinElement({ glyph: "F", background: "#c5221f" }).element }); onDiagnostic("trail_route_rendered", { geometryPointCount: geometry.length,routeSource }); }
