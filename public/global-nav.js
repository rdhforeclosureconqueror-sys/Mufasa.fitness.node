(function installGlobalNavigation(global) {
  ...
  const diagnostics = global.__MAAT_GLOBAL_NAV_DIAGNOSTICS__ = {
    ...
    drawerFound: "NO", backdropFound: "NO", state: "closed", authRole: "signed_out", currentPage: global.location?.pathname || "unknown"
  };
  ...
  function inspect() {
    ...
    diagnostics.authRole=state.user?.role||"signed_out";
  ...
  function render() {
    const wasOpen=diagnostics.state==="open",runtime=global.AuthStateRuntime,state=runtime?.getCanonicalAuthState?.()||{isAuthenticated:false,user:null};
  ...
  function initialize() {
    ...
    global.addEventListener("auth:changed",render);render();inspect();
    global.AuthStateRuntime?.whenReady?.().then(render).catch(inspect);
  }
  global.MaatNavigation={bundle:FRONTEND_BUILD,NAV_ITEMS,render,setOpen,getVisibleItems:state=>NAV_ITEMS.filter(item=>allowed(item,state)),diagnostics};
