(function initWorkoutControlActivation(global) {
  ...
  function bindAvatarModalControls() {
    ...
    file.onchange=()=>{ const selected=file.files?.[0], valid=selected&&/\.glb$/i.test(selected.name||""); diagnostic("avatarDiagFile", selected ? `${valid?"FILE SELECTED":"INVALID FILE"} | name: ${selected.name} | size: ${formatBytes(selected.size)} | browser MIME: ${selected.type || "not provided"}` : "NONE"); diagnostic("avatarDiagCompatibility","NOT CHECKED"); diagnostic("avatarDiagUpload", valid ? "IDLE" : selected ? "FAILED" : "IDLE"); if(selected&&!valid) fail(null,"AVATAR_FILE_EXTENSION"); };
    upload.onclick=async()=>{ if(upload.dataset.pending==="true") return false; const selected=file.files?.[0]||null; upload.dataset.pending="true"; upload.disabled=true; diagnostic("avatarDiagUpload","PREPARING_UPLOAD"); diagnostic("avatarDiagError","NONE"); try { const result=await global.ProfileWriteRuntime?.uploadAvatarFile?.(selected); if(!result?.ok) throw Object.assign(new Error(result?.reason||"Upload did not complete."),{code:result?.reason}); diagnostic("avatarDiagUpload","SUCCESS"); return true; } catch(error) { diagnostic("avatarDiagUpload",error?.uploadTransportSucceeded ? "SUCCESS" : "FAILED"); fail(error,error?.code||"UPLOAD_FAILED"); return false; } finally { upload.dataset.pending="false"; upload.disabled=false; } };
