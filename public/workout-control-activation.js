(function initWorkoutControlActivation(global) {
  ...
  function bindAvatarModalControls() {
    ...
    file.onchange=()=>{ const selected=file.files?.[0]; diagnostic("avatarDiagFile", selected ? `FILE SELECTED | name: ${selected.name} | size: ${formatBytes(selected.size)} | browser MIME: ${selected.type || "not provided"}` : "NONE"); diagnostic("avatarDiagUpload", selected ? "READY" : "IDLE"); };
