(function initCanonicalApiClient(global) {
  ...
  async function request(path, options) {
    ...
    try {
      url = resolve(path); var authToken = options.auth === false ? null : token(); var headers = Object.assign({}, options.body ? { "Content-Type": "application/json" } : {}, options.headers || {}, authToken ? { Authorization: "Bearer " + authToken } : {}); var method = (options.method || "GET").toUpperCase(); var crossOrigin = new URL(url).origin !== global.location?.origin;
      request = new Request(url, { method: method, headers: headers, body: options.body && typeof options.body !== "string" ? JSON.stringify(options.body) : options.body, cache: options.cache || "no-store", credentials: "omit", signal: controller.signal }); dispatched = true;
      var response = await global.fetch(request); var payload = await response.json().catch(function () { return {}; });
