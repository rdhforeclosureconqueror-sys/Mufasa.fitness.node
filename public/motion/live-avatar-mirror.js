(function (root, factory) {
  ...
  function canonicalCoachSpeak(text) {
    const runtime = typeof globalThis !== "undefined" ? globalThis.CoachRuntime : null;
    if (runtime && typeof runtime.speak === "function") {
      Promise.resolve(runtime.speak(String(text || ""), "avatar-calibration")).catch(() => browserSpeak(text));
      return true;
    }
  ...
  class AvatarMirrorCalibration {
    constructor({ now=()=>Date.now(), speak=canonicalCoachSpeak, onCue=()=>{}, ...options }={}) {
      this.now=now;this.speak=speak;this.onCue=onCue;this.options={...CALIBRATION_DEFAULTS,...options};this.state="FRAMING";this.stableFrames=0;this.promptCount=0;this.lastPromptAt=null;this.stateStartedAt=this.now();this.countdownSpoken=new Set();this.lastFrame=null;
    }
    ...
    cue(text){this.onCue(text,this.state);this.speak?.(text);}
    observe(frame,at=Number(frame?.timestamp||this.now())){this.lastFrame=frame;if(this.state==="READY")return true;const inFrame=this.framed(frame);if(this.state==="FRAMING"){
      if(!inFrame){this.stableFrames=0;if(this.lastPromptAt==null||at-this.lastPromptAt>=this.options.promptIntervalMs){if(this.promptCount<2)this.cue("Step back so I can see your full body.");else if(this.promptCount===2)this.cue("I'll resume when you're in position.");this.promptCount++;this.lastPromptAt=at;}return false;}
      this.stableFrames++;if(this.stableFrames>=this.options.stableFrames){this.state="SETTLING";this.stateStartedAt=at;this.cue("I can see you. Get into your base position and hold still.");}return false;
    }
      ...
      if(this.state==="SETTLING"&&at-this.stateStartedAt>=this.options.settleMs){this.state="COUNTDOWN";this.stateStartedAt=at;this.countdownSpoken.clear();this.cue("Taking your base position in 3.");this.countdownSpoken.add(3);return false;}
      if(this.state==="COUNTDOWN"){
        const elapsed=at-this.stateStartedAt,step=this.options.countdownStepMs;
        if(elapsed>=step&&!this.countdownSpoken.has(2)){this.cue("2.");this.countdownSpoken.add(2);}if(elapsed>=step*2&&!this.countdownSpoken.has(1)){this.cue("1.");this.countdownSpoken.add(1);}if(elapsed>=step*3){this.state="CAPTURING";this.stateStartedAt=at;this.cue("Hold.");return true;}return false;
      }
  ...
  class LiveAvatarMirror {
    constructor({ eventTarget, session, cameraState=()=>({}), now=()=>Date.now(), solverOptions={}, stabilizerOptions={}, bodyFollowOptions={}, calibrationOptions={}, speak=canonicalCoachSpeak, activateVoice=activateCanonicalCoachVoice, onCalibrationCue=()=>{}, onPose=()=>{} }={}) {
      if(!eventTarget||!session?.avatar||!session?.THREE)throw new TypeError("eventTarget and a loaded motion session are required");this.eventTarget=eventTarget;this.session=session;this.cameraState=cameraState;this.onPose=onPose;this.now=now;this.disposed=false;this.poseFramesReceived=0;this.retargetFramesExecuted=0;this.lastRetargetAt=null;this.authoritativeTimes=[];this.lastBoundsAt=0;this.boundsProof={avatarWorldBoundsCenter:null,avatarWorldBoundsSize:null};this.voiceActivationState="REQUESTED";this.voiceActivationResult=null;session.unloadMotion?.();this.solver=new solverApi.AvaturnLivePoseSolver({THREE:session.THREE,avatar:session.avatar,now,...LIVE_SOLVER_DEFAULTS,...solverOptions});this.stabilizer=new AvatarPresentationStabilizer({now,...stabilizerOptions});this.bodyFollower=new AvatarBodyFollower({avatar:session.avatar,now,...bodyFollowOptions});this.calibration=new AvatarMirrorCalibration({now,speak,onCue:onCalibrationCue,...calibrationOptions});
      Promise.resolve(activateVoice?.()).then(result=>{this.voiceActivationResult=result||null;this.voiceActivationState=result?.ok===false?"DEGRADED":"ACTIVE";}).catch(error=>{this.voiceActivationResult={ok:false,reason:String(error?.message||error||"coach_voice_activation_failed")};this.voiceActivationState="DEGRADED";});
      this.onFrame=event=>{const camera=this.cameraState()||{},frame=normalized.fromMoveNetPosePacket(event?.detail?.posePacket,{cameraFacing:camera.facingMode,previewMirrored:camera.isMirrored});this.poseFramesReceived+=1;this.authoritativeTimes.push(frame.timestamp);if(this.authoritativeTimes.length>60)this.authoritativeTimes.shift();this.stabilizer.observe(frame);const allowed=this.calibration.observe(frame,frame.timestamp);if(allowed&&this.calibration.captureEnabled())this.bodyFollower.observe(frame);this.onPose(frame,{...this.solver.diagnostics(),...this.calibration.diagnostics(),voiceActivationState:this.voiceActivationState});};eventTarget.addEventListener("pose-runtime:frame",this.onFrame);
    }
  ...
  return Object.freeze({LiveAvatarMirror,AvatarPresentationStabilizer,AvatarMirrorCalibration,AvatarBodyFollower,canonicalCoachSpeak,activateCanonicalCoachVoice,PRESENTATION_DEFAULTS,LIVE_SOLVER_DEFAULTS,BODY_FOLLOW_DEFAULTS,CALIBRATION_DEFAULTS});
});