(function (root, factory) {
  ...
  class AvatarMirrorCalibration {
    constructor({ now=()=>Date.now(), speak=browserSpeak, onCue=()=>{}, ...options }={}) {
  ...
  class LiveAvatarMirror {
    constructor({ eventTarget, session, cameraState=()=>({}), now=()=>Date.now(), solverOptions={}, stabilizerOptions={}, bodyFollowOptions={}, calibrationOptions={}, speak=browserSpeak, onCalibrationCue=()=>{}, onPose=()=>{} }={}) {
      if(!eventTarget||!session?.avatar||!session?.THREE)throw new TypeError("eventTarget and a loaded motion session are required");this.eventTarget=eventTarget;this.session=session;this.cameraState=cameraState;this.onPose=onPose;this.now=now;this.disposed=false;this.poseFramesReceived=0;this.retargetFramesExecuted=0;this.lastRetargetAt=null;this.authoritativeTimes=[];this.lastBoundsAt=0;this.boundsProof={avatarWorldBoundsCenter:null,avatarWorldBoundsSize:null};session.unloadMotion?.();this.solver=new solverApi.AvaturnLivePoseSolver({THREE:session.THREE,avatar:session.avatar,now,...LIVE_SOLVER_DEFAULTS,...solverOptions});this.stabilizer=new AvatarPresentationStabilizer({now,...stabilizerOptions});this.bodyFollower=new AvatarBodyFollower({avatar:session.avatar,now,...bodyFollowOptions});this.calibration=new AvatarMirrorCalibration({now,speak,onCue:onCalibrationCue,...calibrationOptions});
      this.onFrame=event=>{const camera=this.cameraState()||{},frame=normalized.fromMoveNetPosePacket(event?.detail?.posePacket,{cameraFacing:camera.facingMode,previewMirrored:camera.isMirrored});this.poseFramesReceived+=1;this.authoritativeTimes.push(frame.timestamp);if(this.authoritativeTimes.length>60)this.authoritativeTimes.shift();this.stabilizer.observe(frame);const allowed=this.calibration.observe(frame,frame.timestamp);if(allowed&&this.calibration.captureEnabled())this.bodyFollower.observe(frame);this.onPose(frame,{...this.solver.diagnostics(),...this.calibration.diagnostics()});};eventTarget.addEventListener("pose-runtime:frame",this.onFrame);
    }
  ...
  return Object.freeze({LiveAvatarMirror,AvatarPresentationStabilizer,AvatarMirrorCalibration,AvatarBodyFollower,PRESENTATION_DEFAULTS,LIVE_SOLVER_DEFAULTS,BODY_FOLLOW_DEFAULTS,CALIBRATION_DEFAULTS});
});
