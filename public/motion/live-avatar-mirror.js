(function (root, factory) {
  ...
  class AvatarCalibrationSpeechArbiter {
    constructor({ now=()=>Date.now(), ...options }={}){this.now=now;this.options={...SPEECH_DEFAULTS,...options};this.tail=Promise.resolve();this.lastText="";this.lastQueuedAt=0;this.cooldownUntil=0;this.spoken=0;this.skipped=0;this.rateLimited=0;this.active=false;this.activeText="";this.stopGeneration=0;this.stopped=false;this.directFallbackCalls=0;}
    ...
    async speakOne(phrase,generation){if(this.isMuted()){this.skipped++;return{ok:false,skipped:true,reason:"coach_runtime_muted"};}if(generation!==this.stopGeneration||this.stopped)return{ok:false,skipped:true,reason:"stopped"};const at=this.now();if(at<this.cooldownUntil){this.skipped++;return{ok:false,skipped:true,reason:"rate_limited_cooldown"};}this.active=true;this.activeText=phrase;globalThis.__POCKETPT_CALIBRATION_SPEECH_LOCK__=true;const runtime=globalThis?.CoachRuntime;if(runtime&&typeof runtime.speak==="function"){let result=null;try{result=await runtime.speak(phrase,"avatar-calibration",{owner:"avatar_calibration",interruptible:true,timerNeutral:true});}catch(error){result={ok:false,reason:String(error?.message||error||"coach_speech_failed")};}if(result?.cancelled){this.stop("mufasa_stop");return{ok:false,skipped:true,reason:"mufasa_stop"};}if(generation!==this.stopGeneration||this.stopped){this.active=false;this.activeText="";return{ok:false,skipped:true,reason:"stopped"};}if(result?.ok){this.spoken++;this.active=false;this.activeText="";return result;}const reason=String(result?.reason||result?.backendReason||"").toLowerCase();if(reason.includes("429")||reason.includes("rate_limited")||reason.includes("too many requests")){this.rateLimited++;this.cooldownUntil=at+this.options.rateLimitCooldownMs;this.active=false;this.activeText="";return{ok:false,skipped:true,reason:"rate_limited_cooldown"};}if(this.isMuted())this.skipped++;else this.skipped++;this.active=false;this.activeText="";return result||{ok:false,reason:"coach_speech_failed"};}
      this.directFallbackCalls++;const fallback=await browserSpeak(phrase);if(fallback?.ok)this.spoken++;else this.skipped++;this.active=false;this.activeText="";return fallback;}
    ...
    diagnostics(){return Object.freeze({speechQueueSpoken:this.spoken,speechQueueSkipped:this.skipped,speechQueueRateLimited:this.rateLimited,speechCooldownUntil:this.cooldownUntil,speechLockActive:this.active,speechLockText:this.activeText,speechStopped:this.stopped,directFallbackCalls:this.directFallbackCalls});}
  }
  ...
  function activateCanonicalCoachVoice() {
    ...
    try { runtime.stopAllSpeech?.("avatar_calibration_acquire"); } catch (_error) {}
    try { if (runtime.getState?.().listening) runtime.stopListening?.(); } catch (_error) {}
    ...
    return unlock.then(unlocked => {
      if (!unlocked) return { ok:false, reason:runtime.getState?.().lastVoiceError||"audio_unlock_failed" };
      return { ok:true, unlocked:true, listening:{ok:false,skipped:true,reason:"calibration_exclusive"}, calibrationExclusive:true };
    }).catch(error=>({ok:false,reason:String(error?.message||error||"coach_voice_activation_failed")}));
  }

  function resumeCanonicalCoachVoice() {
    const runtime = typeof globalThis !== "undefined" ? globalThis.CoachRuntime : null;
    if (!runtime) return Promise.resolve({ ok:false, reason:"coach_runtime_unavailable" });
    if (runtime.getState?.().muted === true && globalThis.__POCKETPT_EXPLICIT_VOICE_MUTE__ === true) return Promise.resolve({ ok:false, skipped:true, reason:"coach_runtime_muted" });
    if (runtime.getState?.().listening) return Promise.resolve({ ok:true, alreadyListening:true });
    try { const result=runtime.startListening?.(); return Promise.resolve(result||{ok:false,reason:"recognition_unavailable"}); }
    catch(error){ return Promise.resolve({ok:false,reason:String(error?.message||error||"recognition_failed")}); }
  }

  function deriveMirrorRetargetBoundary({ poseFramesReceived=0, calibration=null, solver=null, retargetFramesExecuted=0 }={}) {
    ...
    return Object.freeze({ firstFailingBoundary:"NONE", retargetGate:"OPEN", nextAction:"Retargeting is active; continue real-device visual review." });
  }
  ...
  class LiveAvatarMirror {
    constructor({eventTarget,session,cameraState=()=>({}),now=()=>Date.now(),solverOptions={},stabilizerOptions={},bodyFollowOptions={},calibrationOptions={},speak=canonicalCoachSpeak,activateVoice=activateCanonicalCoachVoice,resumeVoice=resumeCanonicalCoachVoice,onCalibrationCue=()=>{},onPose=()=>{}}={}){if(!eventTarget||!session?.avatar||!session?.THREE)throw new TypeError("eventTarget and a loaded motion session are required");this.eventTarget=eventTarget;this.session=session;this.cameraState=cameraState;this.onPose=onPose;this.now=now;this.disposed=false;this.poseFramesReceived=0;this.calibrationBlockedFrames=0;this.retargetFramesExecuted=0;this.lastRetargetAt=null;this.authoritativeTimes=[];this.lastBoundsAt=0;this.boundsProof={avatarWorldBoundsCenter:null,avatarWorldBoundsSize:null};this.voiceActivationState="REQUESTED";this.voiceActivationResult=null;this.voiceResumeAttempted=false;this.voiceResumeResult=null;session.unloadMotion?.();this.solver=new solverApi.AvaturnLivePoseSolver({THREE:session.THREE,avatar:session.avatar,now,...LIVE_SOLVER_DEFAULTS,...solverOptions});this.stabilizer=new AvatarPresentationStabilizer({now,...stabilizerOptions});this.bodyFollower=new AvatarBodyFollower({avatar:session.avatar,now,...bodyFollowOptions});this.calibration=new AvatarMirrorCalibration({now,speak,onCue:onCalibrationCue,...calibrationOptions});sharedSpeechArbiter.release();globalThis.__POCKETPT_CALIBRATION_SPEECH_LOCK__=true;Promise.resolve(activateVoice?.()).then(result=>{this.voiceActivationResult=result||null;this.voiceActivationState=result?.ok===false?"DEGRADED":"CALIBRATION_EXCLUSIVE";}).catch(error=>{this.voiceActivationResult={ok:false,reason:String(error?.message||error||"coach_voice_activation_failed")};this.voiceActivationState="DEGRADED";});this.resumeVoice=resumeVoice;this.onFrame=event=>{const camera=this.cameraState()||{},frame=normalized.fromMoveNetPosePacket(event?.detail?.posePacket,{cameraFacing:camera.facingMode,previewMirrored:camera.isMirrored});this.poseFramesReceived+=1;this.authoritativeTimes.push(frame.timestamp);if(this.authoritativeTimes.length>60)this.authoritativeTimes.shift();this.stabilizer.observe(frame);const allowed=this.calibration.observe(frame,frame.timestamp);if(allowed&&this.calibration.captureEnabled())this.bodyFollower.observe(frame);if(!this.calibration.captureEnabled())this.calibrationBlockedFrames+=1;if(this.calibration.ready()){sharedSpeechArbiter.release();if(!this.voiceResumeAttempted){this.voiceResumeAttempted=true;Promise.resolve(this.resumeVoice?.()).then(result=>{this.voiceResumeResult=result||null;this.voiceActivationState=result?.ok===false?"DEGRADED":"ACTIVE";}).catch(error=>{this.voiceResumeResult={ok:false,reason:String(error?.message||error||"coach_voice_resume_failed")};this.voiceActivationState="DEGRADED";});}}this.onPose(frame,{...this.solver.diagnostics(),...this.calibration.diagnostics(),...sharedSpeechArbiter.diagnostics(),voiceActivationState:this.voiceActivationState,voiceResumeAttempted:this.voiceResumeAttempted});};eventTarget.addEventListener("pose-runtime:frame",this.onFrame);}
    ...
    diagnostics(){const elapsed=this.authoritativeTimes.length>1?this.authoritativeTimes.at(-1)-this.authoritativeTimes[0]:0,at=this.now(),avatar=this.session.avatar,canvas=this.session.renderer?.domElement||this.session.canvas||null,computed=canvas&&typeof getComputedStyle==="function"?getComputedStyle(canvas):null;if(at-this.lastBoundsAt>=500&&this.session.THREE.Box3){const box=new this.session.THREE.Box3().setFromObject(avatar);if(!box.isEmpty())this.boundsProof={avatarWorldBoundsCenter:Object.freeze(box.getCenter(new this.session.THREE.Vector3()).toArray()),avatarWorldBoundsSize:Object.freeze(box.getSize(new this.session.THREE.Vector3()).toArray())};this.lastBoundsAt=at;}let attached=false;for(let node=avatar;node;node=node.parent)if(node===this.session.scene){attached=true;break}const boundary=deriveMirrorRetargetBoundary({poseFramesReceived:this.poseFramesReceived,calibration:this.calibration,solver:this.solver,retargetFramesExecuted:this.retargetFramesExecuted});return Object.freeze({...this.solver.diagnostics(),...this.stabilizer.diagnostics(),...this.bodyFollower.diagnostics(),...this.calibration.diagnostics(),...sharedSpeechArbiter.diagnostics(),...boundary,calibrationBlockedFrames:this.calibrationBlockedFrames,voiceActivationState:this.voiceActivationState,voiceActivationResult:this.voiceActivationResult,voiceResumeAttempted:this.voiceResumeAttempted,voiceResumeResult:this.voiceResumeResult,...this.boundsProof,authoritativePoseFps:elapsed>0?(this.authoritativeTimes.length-1)*1000/elapsed:0,poseFramesReceived:this.poseFramesReceived,retargetFramesExecuted:this.retargetFramesExecuted,lastRetargetAt:this.lastRetargetAt,avatarCanvasFound:Boolean(canvas),avatarCanvasDisplay:canvas?.style?.display||"",avatarCanvasVisibility:canvas?.style?.visibility||"",avatarCanvasComputedDisplay:computed?.display||null,avatarCanvasComputedVisibility:computed?.visibility||null,avatarModelFound:Boolean(avatar),avatarModelVisible:avatar?.visible!==false,avatarModelAttachedToScene:attached,avatarRoot:avatar});}
    stopCalibrationSpeech(){this.calibration.stopSpeech();}
    dispose(){if(this.disposed)return;this.eventTarget.removeEventListener("pose-runtime:frame",this.onFrame);this.bodyFollower.restore();this.solver.dispose();sharedSpeechArbiter.release();if(!this.voiceResumeAttempted){this.voiceResumeAttempted=true;Promise.resolve(this.resumeVoice?.()).catch(()=>{});}this.disposed=true;}
  }

  return Object.freeze({LiveAvatarMirror,AvatarPresentationStabilizer,AvatarMirrorCalibration,AvatarBodyFollower,AvatarCalibrationSpeechArbiter,deriveMirrorRetargetBoundary,canonicalCoachSpeak,activateCanonicalCoachVoice,resumeCanonicalCoachVoice,PRESENTATION_DEFAULTS,LIVE_SOLVER_DEFAULTS,BODY_FOLLOW_DEFAULTS,CALIBRATION_DEFAULTS,SPEECH_DEFAULTS});
});
