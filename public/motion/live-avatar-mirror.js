(function (root, factory) {
  ...
  class AvatarCalibrationSpeechArbiter {
    constructor({ now=()=>Date.now(), ...options }={}){this.now=now;this.options={...SPEECH_DEFAULTS,...options};this.tail=Promise.resolve();this.lastText="";this.lastQueuedAt=0;this.cooldownUntil=0;this.spoken=0;this.skipped=0;this.rateLimited=0;this.active=false;this.activeText="";this.stopGeneration=0;}
    isMuted(){return Boolean(globalThis?.CoachRuntime?.getState?.().muted);}
    queue(text){const phrase=String(text||"").trim();if(!phrase)return Promise.resolve({ok:false,skipped:true,reason:"empty"});const at=this.now();if(this.isMuted()){this.skipped++;return Promise.resolve({ok:false,skipped:true,reason:"coach_runtime_muted"});}if(phrase===this.lastText&&at-this.lastQueuedAt<this.options.duplicateWindowMs){this.skipped++;return Promise.resolve({ok:false,skipped:true,reason:"duplicate"});}this.lastText=phrase;this.lastQueuedAt=at;const generation=this.stopGeneration;this.tail=this.tail.then(()=>{if(generation!==this.stopGeneration)return{ok:false,skipped:true,reason:"stopped"};return this.speakOne(phrase,generation);}).catch(()=>({ok:false,reason:"speech_queue_failed"}));return this.tail;}
    async speakOne(phrase,generation){if(this.isMuted()){this.skipped++;return{ok:false,skipped:true,reason:"coach_runtime_muted"};}if(generation!==this.stopGeneration)return{ok:false,skipped:true,reason:"stopped"};const at=this.now();if(at<this.cooldownUntil){this.skipped++;return{ok:false,skipped:true,reason:"rate_limited_cooldown"};}this.active=true;this.activeText=phrase;globalThis.__POCKETPT_CALIBRATION_SPEECH_LOCK__=true;const runtime=globalThis?.CoachRuntime;let result=null;if(runtime&&typeof runtime.speak==="function"){try{result=await runtime.speak(phrase,"avatar-calibration");}catch(error){result={ok:false,reason:String(error?.message||error||"coach_speech_failed")};}}if(generation!==this.stopGeneration){this.active=false;this.activeText="";return{ok:false,skipped:true,reason:"stopped"};}if(result?.ok){this.spoken++;this.active=false;this.activeText="";return result;}const reason=String(result?.reason||result?.backendReason||"").toLowerCase();if(reason.includes("429")||reason.includes("rate_limited")||reason.includes("too many requests")){this.rateLimited++;this.cooldownUntil=at+this.options.rateLimitCooldownMs;this.active=false;this.activeText="";return{ok:false,skipped:true,reason:"rate_limited_cooldown"};}if(this.isMuted()){this.skipped++;this.active=false;this.activeText="";return{ok:false,skipped:true,reason:"coach_runtime_muted"};}const fallback=await browserSpeak(phrase);if(fallback?.ok)this.spoken++;else this.skipped++;this.active=false;this.activeText="";return fallback;}
    stop(reason="mufasa_stop"){this.stopGeneration+=1;this.active=false;this.activeText="";this.tail=Promise.resolve();try{globalThis?.CoachRuntime?.stopAllSpeech?.(reason);}catch(_error){}try{globalThis?.speechSynthesis?.cancel?.();}catch(_error){}globalThis.__POCKETPT_CALIBRATION_SPEECH_LOCK__=false;return{ok:true,reason};}
    release(){globalThis.__POCKETPT_CALIBRATION_SPEECH_LOCK__=false;}
    diagnostics(){return Object.freeze({speechQueueSpoken:this.spoken,speechQueueSkipped:this.skipped,speechQueueRateLimited:this.rateLimited,speechCooldownUntil:this.cooldownUntil,speechLockActive:this.active,speechLockText:this.activeText});}
  }
  ...
  class AvatarMirrorCalibration {
    ...
    cue(text,nextState=null){this.onCue(text,this.state);const generation=++this.speechGeneration;this.speechPending=true;this.pendingAdvance=nextState;return Promise.resolve(this.speak?.(text)).catch(()=>({ok:false})).finally(()=>{if(generation!==this.speechGeneration)return;this.speechPending=false;const advance=this.pendingAdvance;this.pendingAdvance=null;if(advance){this.state=advance;this.stateStartedAt=this.now();}});}
