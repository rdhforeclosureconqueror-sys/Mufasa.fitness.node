(function (root, factory) {
  ...
  function phase(id, kind, normalizedTime, rootPosition, pose, extra = {}) {
    return Object.freeze({
      ...
      boneTargets: Object.freeze([
        ...
        // Upper-arm direction is not encoded as a guessed local Euler angle. The semantic
        // direction policy below aligns shoulder->elbow with world up on the actual target rig.
        target("mixamorig:LeftArm", [0, 0, 0]),
        target("mixamorig:RightArm", [0, 0, 0]),
        target("mixamorig:LeftForeArm", [0, 0, 0]),
        target("mixamorig:RightForeArm", [0, 0, 0]),
  ...
  const semanticTargets = Object.freeze([
    Object.freeze({ id:"left_upper_arm_overhead", type:"bone_direction_world", bone:"mixamorig:LeftArm", childBone:"mixamorig:LeftForeArm", worldDirection:Object.freeze([0,1,0]), intent:"upper arm points overhead independent of target rig local axes" }),
    Object.freeze({ id:"right_upper_arm_overhead", type:"bone_direction_world", bone:"mixamorig:RightArm", childBone:"mixamorig:RightForeArm", worldDirection:Object.freeze([0,1,0]), intent:"upper arm points overhead independent of target rig local axes" })
  ]);
  ...
  const spec = Object.freeze({
    ...
    motionId: "assessment/overhead_squat_phase_first_v1_semantic_arms",
    displayName: "Overhead Squat Assessment v1 — Semantic Arm Retarget Test",
    version: 1.1,
    status: "development-test-only",
    movementContractRef: "/motion/contracts/overhead-squat-assessment.v1.json",
    skeleton: Object.freeze({ id:"canonical_phase_e_mixamo", rootBone:"mixamorig:Hips", rotationSpace:"rest_relative_local_plus_semantic_world_direction" }),
    durationSeconds: 9.0,
    loop: false,
    movementContract: Object.freeze({
      ...
      armIntent: "both elbows remain straight while each upper arm points overhead; target-rig bone axes must not determine the meaning of overhead",
      ...
      calibrationRule: "semantic intent first: solve requested body-segment direction from actual loaded rig geometry; use local-axis rotation only for movements whose axis convention is verified"
    }),
    semanticPosePolicy:Object.freeze({
      mode:"target-rig-world-direction",
      coordinateSpace:"world",
      appliesTo:"all phases including generated grounding samples",
      targets:semanticTargets,
      rule:"For each semantic target, orient the loaded rig's bone-to-child segment toward worldDirection before Motion Spec compilation. Never copy a source rig's local Euler axis as if it had the same physical meaning on another rig."
    }),
    ...
    assessmentObservationContract: Object.freeze({
      ...
      checkpoints:Object.freeze(["feet_direction", "knee_tracking", "torso_forward_lean", "lumbar_extension_pattern", "arms_overhead_retention"]),
    ...
    synthesisBoundary: Object.freeze({
      method:"phase-first-assessment-description-plus-owner-reviewed-squat-geometry-plus-semantic-arm-direction",
      lowerBodySource:"/motion/squat-motion-spec.js",
      overheadPoseStatus:"semantic-world-up-owner-visual-verification-required",
      copiedNamedAnimation:false,
      priorFailure:"Copying canonical Mixamo local-X arm rotation onto Avaturn only after renaming the bone caused rig-axis-dependent arm paths through the torso.",
      unsupported:Object.freeze(["medical diagnosis","injury prediction","production scoring thresholds","universal anthropometric fit","claim that world-up alone proves ideal scapular mechanics or exact arms-by-ears spacing"])
    }),
    phaseOrder,
  ...
  function validate(candidate, availableBones = CANONICAL_BONES) {
    ...
    if (semantic.length !== 2) errors.push("OHSA_SEMANTIC_ARMS: exactly two semantic upper-arm direction targets are required");
    for (const item of semantic) {
      if (item.type !== "bone_direction_world") errors.push(`OHSA_SEMANTIC_TYPE: ${item.id || item.bone} must use bone_direction_world`);
      if (!bones.has(item.bone) || !bones.has(item.childBone)) errors.push(`OHSA_SEMANTIC_BONE: unresolved semantic target ${item.id || item.bone}`);
      if (JSON.stringify(item.worldDirection || []) !== JSON.stringify([0,1,0])) errors.push(`OHSA_SEMANTIC_DIRECTION: ${item.id || item.bone} must request world up`);
    }
    ...
    for (const item of phases) {
      ...
      for (const bone of ["mixamorig:LeftArm","mixamorig:RightArm","mixamorig:LeftForeArm","mixamorig:RightForeArm"]) {
        const offset = item.boneTargets?.find(t=>t.bone === bone)?.rotationOffsetEulerDegrees;
        if (JSON.stringify(offset || []) !== JSON.stringify([0,0,0])) errors.push(`OHSA_ARM_LOCAL_AXIS_GUESS_FORBIDDEN: ${bone} must stay rest-relative zero in ${item.id}; semantic direction owns overhead placement`);
      }
    }
  ...
  function summary(candidate = spec) {
    return Object.freeze({
      ...
      semanticDirectionTargetCount:candidate.semanticPosePolicy?.targets?.length || 0,
