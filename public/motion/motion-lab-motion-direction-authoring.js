(function initMotionLabMotionDirectionAuthoring(root, document) {
  ...
  const VERSION = '1.0.1-adjacent-phase-direction-humanize';
  ...
  function restorePlan(plan) { intents = (Array.isArray(plan?.transitions) ? plan.transitions : []).map(item => Object.freeze({ ...item })); workingClip = session()?.sessionClip?.clone?.() || session()?.sessionClip || null; syncPhaseOptions(); status(intents.length ? `Restored ${intents.length} directed transition(s) from the saved authored motion.` : 'No directed transitions were stored in this draft.'); publish({ status: 'ready', firstFailingBoundary: null }); return { status: 'ready', transitionCount: intents.length }; }
  ...
  function install() {
    ...
    el('motionDirectionPreview')?.addEventListener('click', preview); el('motionDirectionClear')?.addEventListener('click', clearPlan);
    el('motionDirectionFromPhase')?.addEventListener('change', updateAvailability); el('motionDirectionToPhase')?.addEventListener('change', updateAvailability);
  ...
  function autoInstall(attempt = 0) {
    ...
    else { status('Motion direction authoring did not initialize. Press Initialize Runtime to retry.', 'failed'); publish({ status: 'failed', firstFailingBoundary: 'MOTION_DIRECTION_INSTALL' }); }
  }
  const initializeButton = el('initializeRuntime');
  initializeButton?.addEventListener('click', () => root.setTimeout?.(() => autoInstall(0), 0));
