(function initMotionLabIntelligenceDiagnostics(root) {
  ...
  let snapshot = Object.freeze({
    ...
    code: null,
    phaseConstraintDiagnostics: Object.freeze([]),
    updatedAt: null
  });
  ...
  function diagnosticsText() {
    ...
    snapshot.phaseConstraintDiagnostics.forEach(phase => {
      lines.push(`  ${phase.phaseId || '?'}: contacts=${phase.contactCount ?? '—'} correction=${phase.correctionStatus || '—'} magnitude=${formatNumber(phase.correctionMagnitudeWorldUnits)} residual=${formatNumber(phase.maxResidualWorldUnits)} firstFailure=${boundaryName(phase.firstFailure, 'NONE') || 'NONE'}`);
    });
  ...
  function ensureHost() {
    ...
    heading.textContent = 'Motion Intelligence — Consolidated Debug';

    const controls = document.createElement('div');
    controls.className = 'controls';
    const copy = document.createElement('button');
    copy.id = 'copyMotionIntelligenceDiagnostics';
    copy.type = 'button';
    ...
    pre.id = 'motionIntelligenceDiagnosticsText';
    ...
    const wrapped = Object.freeze({
      ...
      compile(THREE, spec, avatar) {
        publish({
          ...
          motionId: spec?.motionId || null,
          exerciseId: spec?.exerciseId || null,
          declaredGroundingContacts: Array.isArray(spec?.groundingPolicy?.contacts) ? spec.groundingPolicy.contacts.length : 0,
          contactPhasesDeclared: contactPhaseCount(spec),
          contactPhasesValidated: 0,
          firstFailingPhase: null,
          firstFailingBoundary: null,
          code: null,
          phaseConstraintDiagnostics: Object.freeze([]),
        ...
        try {
          ...
          publish(Object.freeze({
            ...snapshot,
            compileStatus: 'FAIL',
            sharedIntelligenceCore: 'NOT REACHED',
            intelligenceAdapter: 'NOT REACHED',
            kinematicValidation: 'NOT REACHED',
            contactLock: 'NOT REACHED',
            rootContactCorrection: 'NOT REACHED',
            firstFailingBoundary: error?.code || 'compiler_exception',
            code: error?.code || 'compiler_exception',
            updatedAt: new Date().toISOString()
          }));
          throw error;
        }
