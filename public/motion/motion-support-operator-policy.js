(function(root,factory){
  ...
  "use strict";
  const VERSION="1.1.1-body-surface-supports";
  ...
  const SURFACES=Object.freeze({
    ...
    upper_back:Object.freeze({id:"upper_back_surface",bones:Object.freeze(["Spine2"]),mode:"support_plane_y"})
  });
  ...
  function compile(plan,phases){
    ...
    const surfaceSupportPolicy=Object.freeze({
      ...
      constraints:Object.freeze(surfaceRecords.flatMap(surface=>surface.bones.map(bone=>Object.freeze({id:`${surface.id}:${bone}`,sourceSupport:surface.id,bone,axis:"world_y"}))))
    });
    const hasAnchored=contacts.length>0;
    return Object.freeze({status:"ready",phases:nextPhases,groundingPolicy:Object.freeze({mode:hasAnchored?"generated-supports-v2":"description-supports-no-endpoints",contacts:Object.freeze(contacts.slice()),contactBones,anchorPhaseId:hasAnchored?anchorPhaseId:null,kinematicChains:chains,enforceContactAnchors:hasAnchored,enforceGeneratedIK:hasAnchored}),surfaceSupportPolicy,diagnostics:Object.freeze({operatorVersion:VERSION,declaredSupports:Object.freeze((plan.supports||[]).slice()),endpointSupports:classification.endpoint,surfaceSupports:classification.surface,enforcedContacts:Object.freeze(contacts.slice()),promotedSurfaceContacts:Object.freeze([]),surfacePlaneConstraints:Object.freeze(surfaceSupportPolicy.constraints.map(x=>x.id)),activePhaseIds:activeIds,anchorPhaseId,endpointContactSolvingApplied:endpointRecords.length>0,bodySurfaceSolvingApplied:surfaceRecords.length>0,firstDeferredCapability:null})});
  }
