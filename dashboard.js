...
(function () {
  ...
  function renderLaunchHealth(health) {
    ...
    launchHealthSections.replaceChildren(...healthDomains.map((domain) => {
      ...
      const domainChecks = domain === "Launch Readiness" ? [{ id: "overall", status: health.launchReadiness?.status, explanation: `Blockers: ${(health.launchReadiness?.blockers || []).join(", ") || "none"}`, blocking: (health.launchReadiness?.blockers || []).length > 0, lastCheckedAt: health.generatedAt }] : (health.checks || []).filter(item => item.domain === domain);
      const body = document.createElement("div"); body.className = "diagPre";
      body.textContent = domainChecks.length ? domainChecks.map(item => `${item.status || "UNKNOWN"} — ${item.affectedFeature || item.id}\n${item.explanation || ""}\nBlocking: ${item.blocking ? "yes" : "no"}${item.remediation ? `\nRemediation: ${item.remediation}` : ""}\nChecked: ${item.lastCheckedAt || health.generatedAt}`).join("\n\n") : "No applicable Version 1 checks.";
  ...
  copyRepairSummaryBtn?.addEventListener("click", async () => { const health = window.__lastLaunchHealth; const text = `Launch health: ${health?.launchReadiness?.status || "UNKNOWN"}. Blockers: ${(health?.launchReadiness?.blockers || []).join(", ") || "none"}. Warnings: ${(health?.launchReadiness?.warnings || []).join(", ") || "none"}. This summary is redacted.`; await navigator.clipboard.writeText(text); });
