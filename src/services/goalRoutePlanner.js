...
function option({ target, estimated, geometry, routeType, turnaroundPoint = null, loopCount = null, instructions }) {
  return { routeSource:"verified_geometry", routeSourceLabel:"Verified trail route", source:"verified_geometry", sourceLabel:"Verified trail route", routeType, targetDistanceMeters:target, estimatedDistanceMeters:estimated, distanceErrorMeters:estimated-target, distanceErrorPercent:target?Math.abs(estimated-target)/target*100:0, polyline:geometry, startPoint:geometry[0], turnaroundPoint, endPoint:geometry[geometry.length-1], loopCount, warnings:["Follow posted trail signs, closures, and local conditions."], confidence:"high", instructions };
}
