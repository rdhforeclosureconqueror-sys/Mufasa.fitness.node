...
function createSteppingIntoGreatnessService({userStore,clock=()=>Date.now()}) {
  ...
  function complete(userId,input){
    ...
    let result;userStore.updateUser(userId,user=>{const d=ensureDomain(user),existing=d.activities.find(a=>input.clientSessionId&&a.clientSessionId===input.clientSessionId);if(existing){
  ...
  return{join,leave,updateSettings,membership,enroll,challengeList,complete,remove,setEligibility,recalculateDomain,revokeActivityContributions,journey,activity,feed,weeklySummary,route,recordOperationalEvent,validateCompletion};
}
