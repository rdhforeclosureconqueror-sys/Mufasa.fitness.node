...
function createSteppingIntoGreatnessService({userStore,clock=()=>Date.now()}) {
  const audit=(d,eventName,metadata={})=>d.operationalEvents.push({eventId:id("op"),eventName,occurredAt:iso(clock()),metadata});
  ...
  function join(userId,preferences={}){let result;userStore.updateUser(userId,user=>{const d=ensureDomain(user);let m=d.memberships.find(x=>x.communityId==="greatness_movement");const visibilityPreferences={...DEFAULT_VISIBILITY,...preferences};if(m)Object.assign(m,{status:"active",visibilityPreferences,leftAt:null,updatedAt:iso(clock())});else{m={membershipId:id("membership"),communityId:"greatness_movement",userId,status:"active",joinedAt:iso(clock()),visibilityPreferences};d.memberships.push(m);}result=m;recalculateDomain(d,userId);return user;});return result;}
  function updateSettings(userId,preferences){let result;userStore.updateUser(userId,user=>{const d=ensureDomain(user),m=activeMembership(d);if(!m)throw new ApiError("COMMUNITY_MEMBERSHIP_REQUIRED","Join The Greatness Movement to edit settings",403);const allowed=Object.keys(DEFAULT_VISIBILITY);for(const key of Object.keys(preferences||{}))if(!allowed.includes(key))throw new ApiError("INVALID_COMMUNITY_SETTING",`Unsupported setting: ${key}`,400);m.visibilityPreferences={...DEFAULT_VISIBILITY,...m.visibilityPreferences,...preferences};m.updatedAt=iso(clock());result=m;return user;});return result;}
  ...
  function complete(userId,input){
    ...
    let result;userStore.updateUser(userId,user=>{const d=ensureDomain(user),existing=d.activities.find(a=>input.clientSessionId&&a.clientSessionId===input.clientSessionId);if(existing){result={...existing,duplicateCompletion:true};return user;}
  ...
  function journey(userId){const d=ensureDomain(userStore.loadUser(userId)),activities=d.activities.filter(a=>!a.deletedAt).map(a=>({...a,route:{...a.route,points:undefined}}));return{activities,personalBests:d.personalBests,achievements:d.achievements,lifetimeDistanceMeters:eligibleActivities(d).reduce((s,a)=>s+a.distanceMeters,0),activeDayTotal:new Set(eligibleActivities(d).map(a=>a.endedAt.slice(0,10))).size,longestStreakDays:streakFor(eligibleActivities(d)),membership:activeMembership(d)};}
  ...
  return{join,leave,updateSettings,membership,enroll,challengeList,complete,remove,setEligibility,recalculateDomain,revokeActivityContributions,journey,activity,feed,weeklySummary,route,validateCompletion};
}

module.exports={createSteppingIntoGreatnessService,validateCompletion,verificationFor,ensureDomain,CHALLENGES,VERIFICATION_LEVELS};
