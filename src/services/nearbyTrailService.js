...
function createNearbyTrailService({provider,routeStore,ttlMs=Number(process.env.TRAIL_CACHE_TTL_MS)||1_800_000,maxEntries=Number(process.env.TRAIL_CACHE_MAX_ENTRIES)||250,now=Date.now}={}) {
  ...
  return {health:()=>({...((provider.health?.())||{provider:"unknown",configured:false}),cacheStatus:{entries:cache.size,ttlMs,maxEntries}}),async search(_userId,input){const validated=validateSearch(input),key=coarseKey(validated);cleanup();const cached=cache.get(key);if(cached&&now()-cached.savedAt<=ttlMs)return{trails:enrich(cached.trails),searchedAt:new Date(cached.savedAt).toISOString(),locationStored:false,cached:true,stale:false};try{const trails=await provider.searchNearbyTrails(validated);cache.set(key,{trails,savedAt:now()});cleanup();if(!trails.length)throw trailError("TRAIL_SEARCH_NO_RESULTS",404);return{trails:enrich(trails),searchedAt:new Date().toISOString(),locationStored:false,cached:false,stale:false};}catch(error){if(cached&&now()-cached.savedAt<=ttlMs*2)return{trails:enrich(cached.trails),searchedAt:new Date(cached.savedAt).toISOString(),locationStored:false,cached:true,stale:true};throw error;}},_cache:cache};
}
