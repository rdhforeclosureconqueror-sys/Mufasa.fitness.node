...
const CAPABILITY_REGISTRY = Object.freeze([
  ...
  capability("notifications", "In-app Notifications", "Notifications", { requiredFeatureFlags: ["GAMIFICATION_NOTIFICATIONS"], requiredRoutes: ["/api/me/notifications", "/api/me/notifications/unread-count"], requiredPersistence: ["append-only notification projection"], expectedMemberUi: "/dashboard.html#notificationExperience", acceptableDegradedMode: "In-app is the only implemented delivery channel." }),
  capability("universal_leaderboards", "Universal XP Leaderboards", "Leaderboards", { requiredFeatureFlags: ["GAMIFICATION_LEADERBOARDS"], requiredRoutes: ["/api/me/leaderboards", "/api/me/leaderboards/:leaderboardId"], expectedMemberUi: "/dashboard.html#leaderboardExperience" }),
  capability("pushup_leaderboard", "Push-Up Challenge Leaderboard", "Leaderboards", { requiredRoutes: ["/api/challenges/pushup/leaderboard"], optionality: "separate_domain" }),
  capability("greatness_events", "Stepping Into Greatness Events", "Gamification", { requiredFeatureFlags:["GAMIFICATION_EVENT_CAPTURE"], requiredRoutes:["/api/me/greatness/activities"], requiredPersistence:["gamification event store"] }),
  capability("pushup_events", "Push-Up Challenge Events", "Gamification", { requiredFeatureFlags:["GAMIFICATION_EVENT_CAPTURE"], requiredRoutes:["/api/challenges/pushup/results"], requiredPersistence:["gamification event store"] }),
  capability("diagnostic_summarizer", "Diagnostic AI Summarizer", "Diagnostics", { requiredFeatureFlags: ["DIAGNOSTIC_SUMMARIZER_ENABLED"], acceptableDegradedMode: "Deterministic diagnostics remain authoritative." }),
  capability("designated_member_journey", "Designated Member Journey", "Member Journey", { requiredRoutes: ["/api/admin/diagnostics/member-journey"], requiredPermissions: ["ops.read_observability"] }),
