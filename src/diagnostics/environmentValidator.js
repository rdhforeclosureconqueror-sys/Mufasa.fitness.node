...
function validateEnvironment(env = process.env, { profile = env.NODE_ENV || "development" } = {}) {
  ...
  const entries = DEFINITIONS.map(([name, baseRequired, type]) => {
    ...
    const required = baseRequired && production || (ai && ["AI_COACH_PROVIDER", "AI_COACH_MODEL", "OPENAI_API_KEY"].includes(name)) || (billing && ["STRIPE_SECRET_KEY", "STRIPE_WEBHOOK_SECRET", "STRIPE_PRICE_ID"].includes(name));
