...
const STATUS = Object.freeze({ READY: "READY", READY_WITH_LIMITATION: "READY_WITH_LIMITATION", DEGRADED: "DEGRADED", BLOCKED: "BLOCKED", DISABLED_INTENTIONALLY: "DISABLED_INTENTIONALLY", EXCLUDED_FROM_V1: "EXCLUDED_FROM_V1", UNKNOWN: "UNKNOWN" });
...
function classifyBuilds({ frontendBuild, backendBuild }) {
  ...
  return { classification: "frontend_current", frontendClassification: "frontend_current", backendClassification: "backend_current", cacheMismatch: false, compatible: true, frontend, backend };
}
...
function buildLaunchHealth({ env = process.env, rootDir = process.cwd(), dataDir, frontendBuild, backendBuild, memberEvidence = null } = {}) {
  ...
  const notifications = gamification.notificationsFlag
    ? { status: "FLAG_ENABLED_BUT_FEATURE_NOT_IMPLEMENTED", implemented: false, inApp: false, externalDelivery: false, missing: ["notification service", "notification projection", "member read route", "delivery queue"] }
  ...
  const checks = [
    check("build_compatibility", "Deployment", builds.compatible === false ? STATUS.BLOCKED : builds.compatible ? STATUS.READY : STATUS.UNKNOWN, builds.compatible === false ? "Frontend and backend build dates differ; redeploy the frontend and invalidate static caches." : "Build identifiers are compatible or unavailable.", { blocking: builds.compatible === false, evidence: [builds] }),
    ...
    check("ai_coach", "AI Coach", safeBool(env.AI_COACH_ENABLED) ? (env.OPENAI_API_KEY && env.AI_COACH_MODEL ? STATUS.READY_WITH_LIMITATION : STATUS.BLOCKED) : STATUS.DISABLED_INTENTIONALLY, "Static AI Coach configuration only; reachability requires an explicit external check."),
  ...
  return { schemaVersion: 1, generatedAt: checked(), statuses: STATUS, builds, environment, storage: { status: storageReady ? STATUS.READY : STATUS.BLOCKED, configured: Boolean(env.POCKET_PT_DATA_DIR), writable: storageReady, durableClaim: Boolean(env.POCKET_PT_DATA_DIR) }, gamification, rewardSimulation: rewardSimulation(rootDir), notifications, leaderboards, stripe, avatar: { enabled: avatarEnabled, status: avatarEnabled ? STATUS.UNKNOWN : STATUS.DISABLED_INTENTIONALLY, probesSkipped: !avatarEnabled }, memberJourney, capabilities: publicCapabilityRegistry(), checks, launchReadiness: { status: blockers.length ? STATUS.BLOCKED : checks.some(x => x.status === STATUS.READY_WITH_LIMITATION) ? STATUS.READY_WITH_LIMITATION : STATUS.READY, blockers: blockers.map(x => x.id), warnings: checks.filter(x => [STATUS.DEGRADED, STATUS.READY_WITH_LIMITATION, STATUS.UNKNOWN].includes(x.status)).map(x => x.id) } };
}

function redactedExport(report) { return JSON.parse(JSON.stringify(report)); }

module.exports = { STATUS, buildLaunchHealth, classifyBuilds, stripeStatic, rewardSimulation, redactedExport };
