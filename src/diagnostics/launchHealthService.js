...
function buildLaunchHealth({ env = process.env, rootDir = process.cwd(), dataDir, frontendBuild, backendBuild, frontendCommit = null, backendCommit = null, assetCacheToken = null, expectedAssetCacheToken = null, memberEvidence = null, implementations = {} } = {}) {
  ...
  const leaderboards = gamification.leaderboardsFlag
    ? implementations.leaderboards ? { ...implementations.leaderboards, status: implementations.clientEvidence?.universal_leaderboard?.status || STATUS.UNKNOWN_UNTIL_CLIENT_EVIDENCE, implemented: true, rankingServiceStatus: STATUS.READY, persistenceProjectionStatus: implementations.leaderboards.persistenceWritable === false ? STATUS.BLOCKED : STATUS.READY, apiStatus: STATUS.READY, preferenceRouteStatus: STATUS.READY, standingsFetchResult: implementations.clientEvidence?.universal_leaderboard?.status || STATUS.UNKNOWN_UNTIL_CLIENT_EVIDENCE, latestClientContractResult: implementations.clientEvidence?.universal_leaderboard || null, routesRegistered: true, uiPresent: exists(rootDir, "public/leaderboards.js") }
  ...
  const checks = [
    ...
    check("greatness_browser_map", "Greatness", greatness.status, "Greatness persistence, discovery, generation, browser configuration, and client rendering are evaluated independently.", { remediation: greatness.browserMapConfiguration === STATUS.CONFIGURATION_MISSING ? "Verify the proven backend /api/browser-config delivery path before changing service configuration." : greatness.browserMapClient === STATUS.CLIENT_RUNTIME_FAILED ? "Use the sanitized client classification to distinguish loader, Google authorization, and container failures." : "Verify a route in a real supported browser and submit sanitized client evidence." }),
  ...
  const warningStatuses = [STATUS.DEGRADED, STATUS.READY_WITH_LIMITATION, STATUS.UNKNOWN, STATUS.UNKNOWN_UNTIL_CLIENT_EVIDENCE, STATUS.CLIENT_RUNTIME_FAILED, STATUS.CONFIGURATION_MISSING];
  return { schemaVersion: 3, generatedAt: checked(), statuses: STATUS, builds: { ...builds, compatibilityResult: compatibilityStatus, frontendBuild: builds.frontend, backendBuild: builds.backend, frontendCommit, backendCommit, assetCacheToken, expectedAssetCacheToken, staticCacheTokenAligned: cacheTokenAligned, deployedCacheFreshness: "UNKNOWN_UNTIL_CLIENT_EVIDENCE" }, environment, storage: { status: storageReady ? STATUS.READY : STATUS.BLOCKED, configured: Boolean(env.POCKET_PT_DATA_DIR), writable: storageReady, durableClaim: Boolean(env.POCKET_PT_DATA_DIR) }, greatness, gamification, rewardSimulation: rewardSimulation(rootDir), notifications, leaderboards, pushupLeaderboard, restoredEvents, aiCoach: aiStatic(env, "AI_COACH", implementations.externalChecks?.aiCoach), diagnosticSummarizer: aiStatic(env, "DIAGNOSTIC_SUMMARIZER", implementations.externalChecks?.diagnosticSummarizer), stripe, avatar: { enabled: avatarEnabled, status: avatarEnabled ? STATUS.UNKNOWN : STATUS.DISABLED_INTENTIONALLY, probesSkipped: !avatarEnabled }, memberJourney, capabilities: publicCapabilityRegistry(), checks, launchReadiness: { status: blockers.length ? STATUS.BLOCKED : checks.some(x => warningStatuses.includes(x.status)) ? STATUS.READY_WITH_LIMITATION : STATUS.READY, blockers: blockers.map(x => x.id), warnings: checks.filter(x => warningStatuses.includes(x.status)).map(x => x.id) } };
}
