...
function createYouthProgramService({ repository, clock = now }) {
  function participantSession(item) {
    if (!item) return null;
    const deliverable = item.status === 'IN_PROGRESS' && item.readiness && !item.pain_flag && allowedSafety(item.safety_validation);
    return { ...item, blueprint: deliverable ? item.blueprint : null };
  }
  ...
  function start(subject, sessionRef) {
    ...
    const existing = repository.getSession(subject, sessionRef); if (existing) return participantSession(existing);
  ...
  function readiness(subject, sessionRef, input) {
    ...
    return participantSession(repository.updateSession(subject, sessionRef, (item) => {
