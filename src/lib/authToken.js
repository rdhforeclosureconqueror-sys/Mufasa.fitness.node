...
function createAuthTokenLib({
  ...
  const configuration = Object.freeze({
    ...
    keyFingerprint: fingerprint(effectiveSecret)
  });

  function sign(payload) {
    const header = { alg: ALGORITHM, typ: TOKEN_TYPE };
    const unsigned = `${encode(JSON.stringify(header))}.${encode(JSON.stringify(payload))}`;
    return `${unsigned}.${crypto.createHmac("sha256", effectiveSecret).update(unsigned).digest("base64url")}`;
  }
  ...
  function verify(token) {
    ...
    const expected = crypto.createHmac("sha256", effectiveSecret).update(unsigned).digest("base64url");
  ...
  function issueUserToken({ userId, provider = "manual", providerSubject = null, email = null, providerVerified = false, identityClass = "manual_unverified", ttlMs = maxTtlMs }) {
    ...
    return { token, fingerprint: fingerprint(token), expiresAt: payload.exp, issuedAt: payload.iat, userId, provider, providerVerified: payload.providerVerified, identityClass: payload.identityClass, jti: payload.jti, claims: payload };
  }

  return { issueUserToken, verify, configuration, fingerprintToken: fingerprint };
}
