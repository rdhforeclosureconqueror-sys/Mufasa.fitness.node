...
function createMembershipTierBridge(options = {}) {
  ...
  function acceptCourtesyTrial(userId){
    const existing=courtesyTrial(userId);if(existing.accepted)return existing;
    const now=Date.now();userStore.updateUser(userId,user=>{user.courtesyTrial={acceptedAt:now,startedAt:now,endsAt:now+COURTESY_TRIAL_MS,accessLevel:"unleashed",cardRequired:false,source:"pocketpt_no_card_trial_v1"};return user;});return courtesyTrial(userId);
  }
  function effectiveTier(userId){
    const membership=membershipService.getMembership(userId),paidPlanId=resolvePlanIdFromStripePrice(membership.stripePriceId,env),trial=courtesyTrial(userId);
    if(membership.hasAccess)return{planId:paidPlanId,status:membership.status,hasAccess:true,source:"membership",trial,stripePriceIdKnown:Boolean(membership.stripePriceId),legacyOrUnmapped:Boolean(membership.stripePriceId&&!paidPlanId)};
  ...
  function register(app){
    ...
    app.get("/api/me/trial-access",requireCanonicalAuth,(req,res)=>{res.set("Cache-Control","private, no-store");return res.status(200).json({ok:true,data:courtesyTrial(req.auth.userId),requestId:req.requestId||null});});
